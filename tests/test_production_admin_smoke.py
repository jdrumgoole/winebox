"""Post-deploy smoke test for the production admin panel.

Verifies that `https://admin.winebox.app` is reachable from the operator
machine after `invoke deploy` finishes. Catches:

- DNS not resolving (admin A record missing or stale).
- Allowlist misconfigured (this machine's IP isn't in
  `deploy/winebox-admin.toml [production].allow`, so nginx returns 403).
- Admin systemd unit dead (nginx 502).
- TLS cert expired or wrong subject.
- Static assets / login page failing to load.

Run from a machine whose IP is in the prod allowlist (typically the same
machine that ran `invoke deploy`):

    uv run python -m pytest tests/test_production_admin_smoke.py -v
...
ADMIN_URL = "https://admin.winebox.app"


def _allowlisted() -> bool:
    """Return True if the test runner can reach the admin /health endpoint.

    The allowlist is enforced at nginx — non-allowed IPs get 403 before
    the request hits the app. We probe once with a short timeout so
...
pytestmark = pytest.mark.skipif(
    os.environ.get("WINEBOX_PROD_ADMIN_SMOKE") != "1" and not _allowlisted(),
    reason=(
        "Production admin smoke test runs only from an allowlisted operator "
        "machine. Set WINEBOX_PROD_ADMIN_SMOKE=1 to force-run (e.g. from the "
        "production droplet)."
    ),
)
