...
**Notable context:** No application code changes since the last review (2026-06-11). HEAD is at commit `35adaae` (the 2026-06-11 security review merge). Dependency versions unchanged. Today's review applied deeper sub-audits across additional code paths (HTML attribute contexts in JS, MongoDB `$set` field path construction, nginx location-block header inheritance, error response content, and console email logging), revealing findings not caught by prior reviews. All 21 INFO-level findings from previous reviews carried forward; 2 new WARNING-level and 4 new INFO-level findings added.
...
**WARNING-1: Unescaped values in JavaScript `img src` and `data-*` attributes (3 locations)**

- **Files:**
  - `winebox/static/js/app.js:6296` — `<img src="/api/images/${wine.front_label_image_path}" ...>` in `renderMetGrid()`
  - `winebox/static/js/app.js:6766` — `<img src="/api/images/${wine.front_label_image_path}" ...>` in `renderMetPickerList()`
  - `winebox/static/js/price-tracker.js:339` — `<img ... src="' + c.photo_url + '" ...>` in capture card rendering
- **Detail:** These three locations insert values into HTML `src` attributes without `escapeHtml()`. The same `front_label_image_path` field IS escaped in other locations (app.js lines 2817 and 2964), making this an inconsistency. The `alt` attribute on the same `<img>` tags correctly uses `escapeHtml()`. The values are server-generated UUID filenames (from `image_storage.save_image()`) and server-constructed URLs, so exploitation requires database poisoning. However, if a poisoned value contained `" onerror="alert(1)`, it would break out of the `src` attribute.
- **Impact:** Low practical risk — values are always server-generated UUIDs. Defence-in-depth gap.
- **Recommendation:** Add `escapeHtml()` to all three locations, matching the pattern already used at app.js lines 2817 and 2964.

**WARNING-2: Import remap custom field names not sanitised for `.` and `$` characters**
...
- **Code:**
  ...
  ```
- **Detail:** User-supplied custom field names from the column mapping request are interpolated into a MongoDB dot-notation path for `$set` updates without sanitisation. A name like `custom:foo.bar` creates the path `custom_fields.foo.bar`, producing unexpected nested subdocuments. A name like `custom:$set.owner_id` would be rejected by modern MongoDB, but older versions might interpret it differently. The `_validate_remap_targets` function (line 94-108) only checks that the custom name is non-empty.
- **Impact:** Low — the update is always scoped to the user's own wine document via `owner_id` filtering. No cross-user data access possible. But unexpected nested document structures could cause application errors.
- **Recommendation:** Add validation to reject custom field names containing `.`, `$`, or null bytes in both `_validate_remap_targets` and `set_column_mapping`.
...
- **File:** `uv.lock` — pinned at 46.0.7, latest is 48.0.1 (released 2026-06-09)
- **Detail:** All known CVEs (CVE-2026-39892, CVE-2026-34073, CVE-2026-26007) are patched at 46.0.7. The version gap itself is not a vulnerability, but staying current is advisable for defence in depth. Version 48.0.1 ships with OpenSSL 4.0.0.
...
**INFO-22: Console email backends log full email body including verification/reset tokens** *(new)*

- **Files:**
  - `winebox/integrations/regstack_email.py:24-29` — `WineboxConsoleEmailService.send()` logs `message.text` at INFO level
  - `winebox/services/email/console.py:49-67` — `ConsoleEmailService.send_email()` logs `text_content` at INFO level
- **Detail:** Both console email backends log the full email body, which includes email verification URLs and password reset URLs containing JWT tokens. If these tokens appeared in persistent logs, they could be used for account takeover within their validity window.
- **Impact:** Low — console backends are only active when `email_backend="console"` (development/test mode). Production uses SES, which logs only the recipient and subject, not the body. Tokens are time-limited and single-use.
- **Recommendation:** Reduce log level to DEBUG, or redact token query parameters from logged URLs.

**INFO-23: nginx `add_header` in landing page location block overrides HSTS** *(new)*

- **Files:**
  - `deploy/nginx-winebox.conf:100-106` — `location = /static/landing.html` block
  - `deploy/nginx-winebox-oat.conf:76-82` — same
- **Detail:** The landing page location block adds `X-Content-Type-Options` and `X-Frame-Options` but omits `Strict-Transport-Security`. Because nginx's `add_header` in a location block overrides all parent `add_header` directives, the HSTS header from the parent server block is dropped for this specific path. The `/static/` block correctly re-emits HSTS, but the exact-match location takes precedence.
- **Impact:** Low — the landing page is a static marketing page. HTTPS redirect still works via nginx's HTTP-to-HTTPS redirect. HSTS preload protects repeat visitors via the preload list. Only affects first-visit HSTS on this single path.
- **Recommendation:** Add `add_header Strict-Transport-Security "max-age=63072000" always;` to the landing page location block.

**INFO-24: No global 500 exception handler** *(new)*

- **Files:** `winebox/main.py`, `winebox/admin/main.py`
- **Detail:** Neither app registers a global exception handler for unhandled `Exception` or `500` errors. The only custom handler is for `RateLimitExceeded`. FastAPI's default 500 handler returns `{"detail": "Internal Server Error"}` which is safe, but adding a dedicated handler would ensure consistent generic responses regardless of configuration.
- **Impact:** Very low — debug mode is blocked in production by startup validation (`main.py:112-116`), and FastAPI's default is safe.
- **Recommendation:** Add a global exception handler that always returns a generic message and logs the exception server-side.

**INFO-25: `tarfile.extractall()` without `filter` argument in backup script** *(new)*

- **File:** `scripts/mongodb_backup.py:378`
- **Detail:** `tar.extractall(path=dump_dir)` does not pass `filter="data"`, making it theoretically vulnerable to path traversal via crafted tar archives (CVE-2007-4559 pattern). The archive is downloaded from the project's own S3 backup bucket and is not user-supplied.
- **Impact:** Very low — exploitation requires control over the S3 backup bucket, which implies a much larger compromise.
- **Recommendation:** Add `filter="data"` (one-line fix).
...
No dependencies are yanked, compromised, or missing critical patches. No lockfile changes since last review. No supply chain concerns.
...
- Three utility scripts hardcode `TestPassword123!` for ephemeral test users. Previously flagged.
- No secrets found in git history via `git log --diff-filter=A`.

#### 3. Authentication & Authorization Audit

All 115 endpoints across main app (88), admin panel (14), and regstack auth (13) audited:
...
- All regex searches use `re.escape()` before compilation (confirmed across 6 files, 18 locations).
- No unsafe MongoDB operators (`$where`, `$expr`, `$accumulator`, `$function`) in user-facing code.
- All `ObjectId` parsing wrapped in `try/except InvalidId` with proper 400/404 responses (verified across 13 files).
...
- All raw `get_pymongo_collection()` calls maintain proper `owner_id` scoping and use hardcoded field names.
...
- XSS prevention consistent across main app rendering (except WARNING-1 locations above).
...
- No token values logged anywhere in production code paths.
...
- User response models exclude `hashed_password` and internal fields.
...
- No logging of passwords, API keys, or full JWT tokens in production code paths.

#### 10. Git History Review

- Only commit since last review: `35adaae` (merge of 2026-06-11 security review — documentation only).
...
- Single branch, single committer. No unexpected contributors.
...
| :yellow_circle: INFO | 25 |
| :green_circle: CLEAN | 12 categories |

**Date of review:** 2026-06-12
**Reviewer:** Automated security audit (Claude Code)
**Codebase version:** 0.7.85 (commit 35adaae)

### Comparison with Previous Review (2026-06-11)

| Item | 2026-06-11 | 2026-06-12 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 2 | +2 new findings from deeper sub-audits |
| INFO | 21 | 25 | +4 new findings |
| WARNING-1 (unescaped JS attributes) | Not checked | New | Deeper JS attribute-context audit |
| WARNING-2 (custom field name sanitisation) | Not checked | New | Deeper MongoDB `$set` path audit |
| INFO-22 (console email token logging) | Not checked | New | Deeper logging audit |
| INFO-23 (nginx HSTS location override) | Not checked | New | Deeper nginx header inheritance audit |
| INFO-24 (no global exception handler) | Not checked | New | Deeper error handling audit |
| INFO-25 (tarfile extractall filter) | Not checked | New | Deeper script audit |
...
- Today's review applied deeper sub-audits across additional code paths not previously examined in detail:
  - HTML attribute context analysis (vs. prior reviews checking innerHTML only)
  - MongoDB `$set` field path construction analysis
  - nginx location-block header inheritance analysis
  - Error response content analysis
  - Console email logging analysis
  - Backup script safety analysis
- These deeper audits revealed 2 WARNING-level and 4 INFO-level findings that were present but undetected in prior reviews.
- All 21 previous INFO-level findings carried forward unchanged.

### Top 3 Priorities

1. **Fix WARNING-1: Add `escapeHtml()` to 3 unescaped JS attribute locations** — trivial one-line fixes in app.js and price-tracker.js that close a defence-in-depth gap.
2. **Fix WARNING-2: Validate custom field names in import remap** — reject `.` and `$` characters in custom field names in `batch_ops.py`.
