...
**Notable context:** No application code changes since the last review (2026-07-13). HEAD is at `0afc882` (version 0.7.86). Dependency versions unchanged in lockfile. All 39 INFO-level findings from the previous review carried forward. Six new INFO findings added (INFO-40 through INFO-45) discovered through deeper analysis of secondary query patterns, export field exposure, rate limiting gaps, and client-side escaping coverage.
...
- **Status:** Carried forward from 2026-05-28 review; updated 2026-07-15 with RawUploadRow detail.
...
**INFO-40: app.js has two unescaped `front_label_image_path` values in img src attributes** :new:
...
- **Status:** New in 2026-07-15 review.
...
- No code changes since last review. HEAD at `0afc882` (previous security review commit only).
...
| :yellow_circle: INFO | 45 |
| :green_circle: CLEAN | 12 categories |

**Date of review:** 2026-07-15
**Reviewer:** Automated security audit (Claude Code)
**Codebase version:** 0.7.86 (commit 0afc882)

### Comparison with Previous Review (2026-07-13)

| Item | 2026-07-13 | 2026-07-15 | Status |
|------|-----------|-----------|--------|
...
- No application code changes since last review (HEAD at `0afc882`, previous security review commit only).
- All dependency versions unchanged in lockfile.
- All 39 INFO findings carried forward unchanged.
- **1 existing finding updated** (INFO-10): added detail about the `RawUploadRow` cleanup querying by the wrong field — see new INFO-43 for the full analysis.
- **6 new INFO findings** added through deeper analysis:
  - **INFO-40**: Two unescaped `front_label_image_path` values in `app.js` (lines 6296, 6766) — inconsistent with the escaped usage at line 2817.
  - **INFO-41**: Weak `escapeHtml()` in `price-tracker.js` (3-character vs 5-character escaping) — separate from the unescaped values in INFO-28.
  - **INFO-42**: Reference data endpoints (`/api/reference/*`) have no per-route rate limiting (13 endpoints) — the global 60/minute limit still applies.
  - **INFO-43**: Admin `delete_user` queries `RawUploadRow` by `owner_id` but the collection uses `batch_id` — cleanup matches zero documents.
  - **INFO-44**: Three secondary wine lookups in `cellar_event_view.py` and `export.py` omit `owner_id` from the filter — defence-in-depth gap.
  - **INFO-45**: Wine export includes internal fields `import_batch_id` and `custom_fields_text` that serve no user purpose.
- Full re-audit of all 12 categories confirms no regressions.

### Top 3 Priorities

1. **Batch dependency upgrade (INFO-33, INFO-34, INFO-26, INFO-38, INFO-18)** — cryptography (GHSA-537c-gmf6-5ccf), python-multipart (3 CVEs), PyJWT (5 CVEs), and Pillow (4 CVEs) are the highest-value upgrades. All are straightforward version bumps with no expected breaking changes. This single maintenance session would close 13 CVEs.
