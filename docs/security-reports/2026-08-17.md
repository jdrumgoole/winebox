...
**Notable context:** Two commits since the last review (2026-08-06): `60acc58` adds a new `prod-ssl` invoke task for renewing the apex landing-page Let's Encrypt cert (with certbot renewal hooks) and removes the dead `auto-merge-security-clean.yml` GitHub Actions workflow; `83d036f` added the 2026-08-06 security report. No application code in `winebox/` has changed. HEAD is at `60acc58`, version 0.7.86. Dependency versions unchanged in lockfile.

**New findings this review:** One WARNING (WAR-1: `SlowAPIMiddleware` not registered — global rate limit is dead code) and one new INFO (INFO-57: CSV/Excel formula injection in export files). WAR-1 corrects an incorrect assumption carried in all prior reports that a global 60/minute per-IP rate limit was effective — it is not, because the middleware that enforces `default_limits` on undecorated routes was never added. INFO-42 has been updated accordingly.
...
**WAR-1: `SlowAPIMiddleware` not registered — `default_limits=["60/minute"]` is dead code for undecorated routes**

- **File:** `winebox/main.py:30-34`
- **Detail:** The main app configures `Limiter(default_limits=["60/minute"])` and sets `app.state.limiter = limiter`, but never registers `SlowAPIMiddleware` (or `SlowAPIASGIMiddleware`). Without the middleware, slowapi only enforces rate limits on routes that have an explicit `@limiter.limit(...)` decorator. The `default_limits` parameter is designed to apply to all undecorated routes via the middleware — without it, the configuration is dead code.
- **Affected routes (authenticated, but zero rate limiting):**
  - `winebox/routers/cellar.py` — inventory, summary, grouped
  - `winebox/routers/met.py` — list, summary
  - `winebox/routers/transactions.py` — list, get
  - `winebox/routers/bottles.py` — add, list, events
  - `winebox/routers/cases.py` — add, list, get, events
  - `winebox/routers/images.py` — label image serving
  - `winebox/routers/wines/crud.py`, `add_to_cellar.py`, `checkout.py`, `grapes.py`, `met.py`, `record.py`, `scores.py`
  - `winebox/routers/price_tracker.py` — list/get/delete/photo (only `create` is decorated)
  - `winebox/routers/demo.py` — `status` and `remove` (only `install` is decorated)
- **Affected routes (unauthenticated AND zero rate limiting):**
  - `winebox/routers/reference.py` — all 13 reference data endpoints (wine types, grape varieties, regions, classifications, region tree)
- **Impact:** An unauthenticated attacker can hit `/api/reference/regions/tree` and other reference endpoints at unlimited rate with no authentication required, potentially exhausting MongoDB connections. Authenticated undecorated routes are also unrate-limited, but the auth requirement provides meaningful access control. Nginx has no `limit_req` configured to backstop this.
- **Correction:** All prior security reports (2026-04-03 through 2026-08-06) stated "Global: 60 requests/minute per IP" in the CLEAN section — this was incorrect. The 60/minute limit only applies additively to the ~11 endpoints with explicit `@limiter.limit()` decorators.
- **Recommendation:** Add `from slowapi.middleware import SlowAPIMiddleware` and `app.add_middleware(SlowAPIMiddleware)` to `winebox/main.py` (after `app.state.limiter = limiter`). This single line will activate the `default_limits=["60/minute"]` for all undecorated routes, matching the clearly intended configuration. Also add to `winebox/admin/main.py` if applicable.
- **Status:** New finding — 2026-08-17. Root cause likely present since rate limiting was first added.
...
- **Detail:** These fields accept unbounded input at the Pydantic layer. All are behind `RequireAuth` or `RequireAdmin`, and most are bounded by application logic elsewhere. The global rate limit (60/min on decorated routes only — see WAR-1) and MongoDB socket timeout (30s) provide partial baseline protection.
- **Impact:** Very low — attacker must be authenticated, and practical exploitation is limited by timeouts.
- **Status:** Carried forward from 2026-05-28 review. Updated 2026-08-17 to correct rate-limit baseline claim per WAR-1.
...
- **Detail:** The `POST /api/wines/record` and `POST /api/wines/met` endpoints can trigger Claude Vision API calls (expensive external service), but have no explicit `@limiter.limit()` decorator. The `/api/wines/scan` endpoint has an explicit 20/minute limit for the same Vision functionality. Per WAR-1, these endpoints have zero rate limiting since the middleware is missing.
- **Impact:** Low — an authenticated user could consume unlimited Vision API quota. Previously rated as "60 requests/minute vs the 20/minute cap" but the actual gap is wider: zero vs 20/minute.
- **Recommendation:** Add explicit rate limits matching the scan endpoint (20/minute; 200/hour).
- **Status:** Carried forward from 2026-05-28 review. Updated 2026-08-17 to correct severity per WAR-1.
...
- **Recommendation:** Prioritize this upgrade alongside the existing INFO-7 version gap recommendation. See also INFO-51 for two additional CVEs requiring 49.0.0.
...
- **CVEs:**
  - **CVE-2026-53540** (Negative Content-Length) — Limited impact on WineBox: nginx likely rejects negative `Content-Length` before it reaches the application.
  - **CVE-2026-53538** (Semicolon as field separator) — Limited impact on WineBox: form endpoints use typed `Form()` parameters with Pydantic validation.
...
- **CVEs:**
  - **CVE-2026-54282** (Path without leading "/" shifts authority boundary) — Not exploitable in WineBox.
...
- **Upgrade path:** Same as INFO-8 — requires upgrading FastAPI from 0.128.1 to >=0.135.0.
- **Status:** Carried forward from 2026-07-06 review.
...
- **Impact on WineBox: None.** WineBox processes only user-uploaded wine label photos via magic byte validation (JPEG, PNG, GIF, WebP only). None of the vulnerable code paths are reachable.
...
- **Detail:** The main application code properly sets timeouts. The missing timeouts are limited to developer utility scripts.
...
- **Detail:** The same field IS properly escaped at line 2817, indicating this is an inconsistency. The values are server-generated paths. CSP blocks inline script execution.
...
- **Impact:** Very low — values used in attribute contexts are server-generated.
- **Status:** Carried forward from 2026-07-15 review.

**INFO-42: Reference data endpoints have no rate limiting (13 endpoints) — severity increased per WAR-1**

- **File:** `winebox/routers/reference.py`
- **Detail:** All 13 reference data endpoints (wine types, grape varieties, regions, classifications) have no per-route rate limits AND are unauthenticated. Per WAR-1, the `default_limits=["60/minute"]` previously assumed to provide baseline protection is dead code — `SlowAPIMiddleware` was never registered. These endpoints have zero rate limiting of any kind.
- **Impact:** Medium — an unauthenticated attacker can hit expensive tree-building queries (`GET /api/reference/regions/tree`, `GET /api/reference/regions/{id}/path`) at unlimited rate with no authentication. MongoDB socket timeout (30s) is the only backstop. Previously rated as low-impact under the false assumption of a global 60/minute limit.
- **Recommendation:** Either add explicit `@limiter.limit()` decorators to reference endpoints (e.g. 120/minute, 2000/hour), or register `SlowAPIMiddleware` per WAR-1 to apply the configured global default.
- **Status:** Carried forward from 2026-07-15 review. Updated 2026-08-17 — severity increased per WAR-1.
...
- **Detail:** Wine IDs come from the user's own CellarEvent/Transaction documents, so in practice they always resolve to the user's own wines. Defence-in-depth improvement.
...
- **Detail:** JSON/YAML wine export includes `import_batch_id` and `custom_fields_text` — internal tracking metadata and search-index denormalization. Non-sensitive but unnecessary in user exports.
- **Status:** Carried forward from 2026-07-15 review.
...
- **CVE:** CVE-2026-53537 — The `parse_options_header` function transparently applies RFC 2231/5987 decoding.
- **Impact on WineBox: Limited.** File upload endpoints validate via magic bytes and replace filenames with UUIDs.
...
- **CVE:** CVE-2026-53533 — SMTP command injection via CR/LF in email addresses.
- **Impact on WineBox: Limited.** Email addresses are validated by Pydantic's `EmailStr`, which rejects CR/LF. Production uses SES via API, not SMTP.
- **Status:** Carried forward from 2026-07-16 review.
...
- **Impact on WineBox: None.** WineBox does not call `click.edit()`. click is used by uvicorn for CLI argument parsing only.
- **Status:** Carried forward from 2026-07-16 review.
...
- **Detail:** Both `ClaudeVisionService.analyze_label()` (async def at line 98) and `match_wines_batch()` (async def at line 115) use synchronous `anthropic.Anthropic()` clients. The synchronous call blocks the event loop for up to 60 seconds.
...
- **CVEs:**
  - **CVE-2026-69248** (CVSS 3.1) — X.509 wildcard DNS name vs permittedSubtrees escape.
  - **CVE-2026-69249** — Duplicate self-signed intermediate certificates cause exponential recursion.
- **Impact on WineBox: None.** WineBox does not directly validate X.509 certificates with name constraints.
- **Upgrade path:** Bump cryptography to >=49.0.0. Supersedes INFO-33's >=48.0.1 recommendation.
...
- **Detail:** The `winebox.app` landing page is served directly by nginx, not proxied to the FastAPI app. The `SecurityHeadersMiddleware` CSP header doesn't apply. The landing page is static marketing content with no dynamic input.
- **Impact:** Very low — no attack surface on the landing page.
- **Status:** Carried forward from 2026-08-05 review.
...
- **Detail:** Both are in archived/test code only, not production code paths.
- **Status:** Carried forward from 2026-08-05 review.
...
- **Detail:** All list endpoints except X-Wines search (`xwines.py:482` has `le=10000`) accept `skip` with no maximum. Owner-scoped queries, and MongoDB socket timeout (30s) provide backstops.
- **Status:** Carried forward from 2026-08-06 review.

**INFO-57: CSV/Excel formula injection in export files — self-injection only**

- **Files:**
  - `winebox/services/export_service/wines.py:23-50` — `_wine_to_base_row()` writes user-controlled fields (`wine.name`, `wine.winery`, `wine.provenance`, custom field values) directly to CSV/XLSX cells without sanitizing formula-triggering prefixes (`=`, `+`, `-`, `@`, `\t`, `\r`).
  - `winebox/services/export_service/transactions.py:23-40` — `_transaction_to_row()` writes user-controlled fields (`txn.notes`, `txn.wine_name`, `txn.provenance_at_event`) directly to CSV/XLSX cells without sanitization.
- **Detail:** A value starting with `=` (e.g. `=cmd|'/c calc'!A1`) stored in a wine name or note field will be interpreted as a live formula when the exported CSV/XLSX file is opened in Excel or Google Sheets, potentially leading to code execution on the machine opening the file. However, WineBox exports are personal — each user exports only their own data (`owner_id`-scoped queries in `export.py`). There is no admin export or shared export feature. An attacker would need to inject formula values into their own wine records and then open their own exported file — a self-injection scenario.
- **Impact:** Very low — self-injection only. No path for one user's data to appear in another user's export.
- **Recommendation:** Prefix cell values starting with `=`, `+`, `-`, `@`, tab, or CR with a single quote (`'`) before writing to CSV/XLSX. Centralise as a helper in `winebox/services/export_service/constants.py`.
- **Status:** New finding — 2026-08-17.

**INFO-58: Rate limit counters use in-memory storage, not shared across workers**

- **File:** `winebox/main.py:30-34` — `Limiter(key_func=get_remote_address, default_limits=["60/minute"])` uses the default `storage_uri="memory://"`.
- **Detail:** slowapi defaults to in-memory storage when no `storage_uri` is configured. Production runs 2 uvicorn workers (`deploy/setup.py:39`), each with an independent counter. A client load-balanced across workers can approximately double any configured rate limit. This affects all explicitly decorated endpoints (search, export, scan, xwines, enrichment, etc.) and would also affect the global default if WAR-1 is fixed.
- **Impact:** Low — limits are degraded but not eliminated. With 2 workers, effective limits are roughly 2x the configured values.
- **Recommendation:** For accurate rate limiting across workers, configure a shared Redis storage: `Limiter(storage_uri="redis://localhost:6379")`. Alternatively, accept the 2x margin as adequate for the current scale.
- **Status:** New finding — 2026-08-17.
...
- **Decorated routes** are correctly rate-limited: search (120/min, 2000/hr), export (10/min, 30/hr), scan (20/min, 200/hr), xwines (120/min, 2000/hr), enrichment (20/min, 100/hr), demo install (5/min, 10/hr), price create (30/min, 200/hr), import (10/min, 30/hr).
- **Auth routes** are correctly rate-limited via regstack's direct per-route mechanism (independent of middleware): login (30/min, 200/hr + 5-attempt lockout), registration (10/min, 50/hr), password reset/change (5/min, 20/hr).
- All paginated queries bounded by `MAX_USER_RESULTSET` (10,000) and `MAX_REFERENCE_RESULTSET` (5,000).
- Database query timeouts configured (serverSelection 5s, connect 10s, socket 30s).
- Upload size bounded by config and nginx.
- **Gap:** Undecorated routes have zero rate limiting — see WAR-1.
...
- Two commits since last review: `60acc58` (prod-ssl task + workflow removal) and `83d036f` (2026-08-06 security report). HEAD at `60acc58`.
- The new `prod-ssl` task in `tasks.py` reviewed: uses `_resolve_prod_host()` (no hardcoded IPs), hardcoded domain constant tuple, `run_ssh` from `deploy.common`, `try/finally` ensures nginx restarts. No injection vectors or security concerns.
- Removed `.github/workflows/auto-merge-security-clean.yml` — was a convenience workflow for auto-merging clean security PRs. No security impact from removal.
...
| Item | 2026-08-06 | 2026-08-17 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 1 | +1 new (WAR-1) |
| INFO | 56 | 58 | +2 new (INFO-57, INFO-58), INFO-42 updated |

### Changes Since Last Review

- Two commits since 2026-08-06: new `prod-ssl` invoke task for renewing the apex landing-page cert, and removal of the `auto-merge-security-clean.yml` GitHub Actions workflow. No application code in `winebox/` changed.
- All dependency versions unchanged in lockfile.
- **New WARNING (WAR-1):** `SlowAPIMiddleware` never registered — the `default_limits=["60/minute"]` configuration is dead code for undecorated routes. This corrects an incorrect assumption carried forward in all prior reports. Specifically, the 13 unauthenticated reference endpoints and ~30 authenticated endpoints have zero rate limiting. The fix is a single line: `app.add_middleware(SlowAPIMiddleware)`.
- **New INFO (INFO-57):** CSV/Excel formula injection in export files — self-injection only since users export only their own data.
- **New INFO (INFO-58):** Rate limit counters use in-memory storage, not shared across the 2 production workers — effective limits are roughly 2x configured values.
- **Updated INFO-42:** Severity increased — reference endpoints previously described as "relying on global 60/minute limit" actually have zero rate limiting.
- **Updated INFO-11, INFO-12:** Notes corrected to reference WAR-1 instead of claiming global rate limit baseline.
- 55 other INFO findings carried forward unchanged.

### Top 3 Priorities

1. **Register `SlowAPIMiddleware` (WAR-1)** — Single-line fix (`app.add_middleware(SlowAPIMiddleware)`) activates the already-configured global 60/minute rate limit for all undecorated routes, closing the widest defence gap found in this review. The 13 unauthenticated reference endpoints are the most exposed.
2. **Batch dependency upgrade (INFO-51, INFO-33, INFO-34, INFO-26, INFO-46, INFO-38, INFO-47, INFO-18, INFO-48, INFO-49)** — cryptography (3 advisories), python-multipart (4 CVEs), PyJWT (5 CVEs), Pillow (7 CVEs), aiosmtplib (1 CVE), and click (1 CVE) are the highest-value upgrades. This single maintenance session would close 21 CVEs.
