...
**Notable context:** One infrastructure commit since the last review (2026-08-06): `60acc58` adds an `invoke prod-ssl` task for apex cert renewal and removes the dead `auto-merge-security-clean.yml` GitHub Actions workflow. No application code or dependency changes. HEAD is at `60acc58` (version 0.7.86). All 56 INFO findings carried forward unchanged.
...
- One commit since last review: `60acc58` — adds `invoke prod-ssl` task for apex cert renewal and removes dead `auto-merge-security-clean.yml` GitHub Actions workflow. No security concerns.
...
| Item | 2026-08-06 | 2026-08-14 | Status |
|------|-----------|-----------|--------|
...
- One infrastructure commit (`60acc58`): adds `invoke prod-ssl` task for apex cert renewal with certbot renewal hooks; removes dead `auto-merge-security-clean.yml` GitHub Actions workflow. No security concerns — the task follows the same `run_ssh` + `certbot` pattern as the existing SSL tasks, properly uses try/finally to restart nginx even on failure, and introduces no new secrets or configuration surface.
