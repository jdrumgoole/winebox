...
**Notable context:** No application code changes since the last review (2026-07-06). HEAD is at `7d29a54` (version 0.7.86). Dependency versions unchanged in lockfile. All 37 INFO-level findings from the previous review carried forward. One new INFO-level finding added for two newly disclosed CVEs in Pillow (CVE-2026-54060 + CVE-2026-55380), and INFO-27 updated with an additional pydantic-settings CVE (CVE-2026-58203).
...
- **CVEs:**
  ...
  - **CVE-2026-58203** — `NestedSecretsSettingsSource` follows symlinks outside `secrets_dir` when `secrets_nested_subdir=True`, enabling local file read and bypassing `secrets_dir_max_size`. :new:
...
- **Recommendation:** Plan upgrade in a future maintenance cycle.
- **Status:** Carried forward from 2026-06-21 review; CVE-2026-58203 added 2026-07-09.
...
**INFO-38: Pillow 12.2.0 is affected by CVE-2026-54060 + CVE-2026-55380 (decompression bomb bypasses) — no practical impact on WineBox** :new:
...
- **Impact on WineBox: None.** WineBox processes only user-uploaded wine label photos. The image pipeline validates uploads via magic byte detection and only accepts JPEG, PNG, GIF, and WebP formats — GD format files are rejected at upload. WineBox does not process font files at any point. Neither vulnerable code path is reachable through WineBox's image handling.
- **Upgrade path:** Bump `pillow>=12.3.0` in pyproject.toml. No breaking changes expected.
- **Recommendation:** Upgrade to 12.3.0 in next maintenance cycle. The upgrade is straightforward and closes both CVEs even though neither is exploitable today.
- **Status:** New in 2026-07-09 review.
...
| pillow | >=12.2.0 | 12.2.0 | 2 new CVEs — no practical impact (see INFO-38) |
...
| Item | 2026-07-06 | 2026-07-09 | Status |
|------|-----------|-----------|--------|
...
- **1 new INFO finding** (INFO-38) added for 2 newly disclosed CVEs in Pillow:
  - INFO-38: Pillow 12.2.0 — CVE-2026-54060 (FontFile decompression bomb bypass) + CVE-2026-55380 (GdImageFile memory exhaustion). Neither is exploitable via WineBox's image handling pipeline (only JPEG/PNG/GIF/WebP accepted; no font file processing).
- **1 existing finding updated** (INFO-27): added CVE-2026-58203 (pydantic-settings symlink traversal) alongside the existing CVE-2026-32690. Same assessment: not applicable to WineBox.
- Full re-audit of all 12 categories confirms no regressions.

### Top 3 Priorities

1. **Batch dependency upgrade (INFO-33, INFO-34, INFO-26, INFO-38)** — cryptography (GHSA-537c-gmf6-5ccf), python-multipart (3 CVEs), PyJWT (5 CVEs), and Pillow (2 CVEs) are the highest-value upgrades. All are straightforward version bumps with no expected breaking changes. This single maintenance session would close 11 CVEs.
