...
**Notable context:** No application code changes since the last review (2026-06-05). HEAD is at commit `dc654b3` (the 2026-06-05 security review merge). Dependency lockfile unchanged. CVE databases re-checked; **two new advisories found** affecting PyJWT and aiohttp. All 17 INFO-level findings carried forward unchanged.
...
**WARNING-1: CVE-2026-48526 — PyJWT algorithm-confusion vulnerability (locked version affected)**

- **File:** `pyproject.toml` line 35, `uv.lock` (pyjwt 2.12.1)
- **CVE:** [CVE-2026-48526](https://cvereports.com/reports/CVE-2026-48526) — When an application decodes tokens using a raw JSON Web Key (JWK) string while simultaneously supporting mixed algorithm families (symmetric and asymmetric), PyJWT does not validate that the key matches its intended algorithm context. An attacker with access to the public JWK can sign forged tokens using the JWK string as an HMAC symmetric secret.
- **Fixed in:** PyJWT 2.13.0
- **Current pin:** `pyjwt>=2.12.0` (locked at 2.12.1)
- **Impact on WineBox: None in current architecture.** WineBox uses HS256 only via regstack with a single symmetric secret key. No JWK strings or mixed algorithm families are used. The `winebox/` codebase has zero imports of JWK or mixed-algorithm configurations. However, PyJWT is in the authentication critical path, and future refactoring could introduce the conditions for exploitation.
- **Recommendation:** Bump pin to `pyjwt>=2.13.0` and run `uv lock --upgrade-package pyjwt` to pull in the fix. This is a one-line change with no breaking API changes.

**WARNING-2: CVE-2026-34993 + CVE-2026-47265 — aiohttp pickle deserialization RCE and cookie disclosure (locked version affected)**

- **File:** `pyproject.toml` line 50, `uv.lock` (aiohttp 3.13.5)
- **CVE-2026-34993:** [Deserialization of untrusted data](https://advisories.gitlab.com/pypi/aiohttp/CVE-2026-34993/) — `CookieJar.load()` uses Python's `pickle` module. A malicious pickle file can execute arbitrary code when loaded. CVSS 6.4 (Medium).
- **CVE-2026-47265:** [Cross-origin redirect cookie leak](https://advisories.gitlab.com/pypi/aiohttp/CVE-2026-47265/) — Per-request cookies set via the `cookies` parameter are not cleared on cross-origin redirects, potentially leaking session tokens to attacker-controlled domains. CVSS 6.6 (Medium).
- **Fixed in:** aiohttp 3.14.0
- **Current pin:** `aiohttp>=3.13.4` (locked at 3.13.5)
- **Impact on WineBox: None.** aiohttp is pinned as a transitive dependency only (not directly imported by WineBox code). WineBox does not use `CookieJar.load()` or per-request cookies with the aiohttp client. Grep confirms zero matches for `CookieJar` or `aiohttp.client` in the `winebox/` directory.
- **Recommendation:** Bump pin to `aiohttp>=3.14.0` and run `uv lock --upgrade-package aiohttp` to pull in both fixes. This is a one-line change. The existing CVE tracking comment should be updated to include all four patched CVEs.
...
All key dependencies are at patched versions for previously-known CVEs. Two new advisories flagged as WARNING-1 and WARNING-2 above.
...
| pyjwt | >=2.12.0 | 2.12.1 | **CVE-2026-48526 NEW — see WARNING-1** |
...
| posthog | >=7.0.0 | 7.13.0 | No known 2026 CVEs (May 2026 supply chain incident affected posthog-js, not Python SDK) |
| cairosvg | >=2.9.0 | 2.9.0 | Patched for CVE-2026-31899 (see INFO-5 re unused) |
| aiohttp | >=3.13.4 | 3.13.5 | **CVE-2026-34993 + CVE-2026-47265 NEW — see WARNING-2** |
...
No dependencies are yanked, compromised, or missing critical patches (beyond the two WARNING items above). No supply chain concerns — none of the 2026 compromised packages (LiteLLM, PyTorch Lightning, durabletask) are WineBox dependencies.
...
| Item | 2026-06-05 | 2026-06-06 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 2 | **+2 new** |
| INFO | 17 | 17 | No change |
| WARNING-1 (PyJWT CVE-2026-48526) | — | NEW | New CVE published |
| WARNING-2 (aiohttp CVE-2026-34993 + CVE-2026-47265) | — | NEW | New CVEs published |
| INFO-1 through INFO-17 | Carried forward | Carried forward | No change |

### Changes Since Last Review

- No code changes since last review. All dependency lock versions unchanged.
- **Two new CVE advisories discovered** affecting locked versions of PyJWT (2.12.1) and aiohttp (3.13.5).
- CVE-2026-48526 (PyJWT algorithm confusion) — not exploitable in WineBox's current HS256-only architecture, but the pin should be bumped.
- CVE-2026-34993 (aiohttp pickle deserialization) and CVE-2026-47265 (aiohttp cookie leak) — not exploitable because aiohttp is a transitive dependency with no direct usage, but the pin should be bumped.
- All 17 INFO-level findings carried forward unchanged.
- Full re-audit of all 12 categories confirms no regressions beyond the new CVEs.

### Top 3 Priorities

1. **Bump PyJWT pin to >=2.13.0 (WARNING-1)** — trivial one-line fix to cover CVE-2026-48526 algorithm confusion. Not exploitable today, but PyJWT is in the authentication critical path.
2. **Bump aiohttp pin to >=3.14.0 (WARNING-2)** — trivial one-line fix to cover CVE-2026-34993 (pickle RCE) and CVE-2026-47265 (cookie leak). Not exploitable because aiohttp is a transitive dependency only.
