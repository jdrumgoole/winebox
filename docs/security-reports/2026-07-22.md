...
**Notable context:** No application code changes since the last review (2026-07-16). HEAD is at `aac1a9b` (version 0.7.86). Dependency versions unchanged in lockfile. All 49 INFO-level findings from the previous review carried forward. One new INFO finding added (INFO-50) discovered through deeper analysis of Anthropic client usage patterns.
...
**INFO-50: Synchronous Anthropic client used in async functions blocks the event loop** :new:
...
- **Status:** New in 2026-07-22 review.
...
- No code changes since last review. HEAD at `aac1a9b` (previous security review commit only).
...
**Codebase version:** 0.7.86 (commit aac1a9b)

### Comparison with Previous Review (2026-07-16)

| Item | 2026-07-16 | 2026-07-22 | Status |
|------|-----------|-----------|--------|
...
- No application code changes since last review (HEAD at `aac1a9b`, previous security review commit only).
- All dependency versions unchanged in lockfile.
- All 49 INFO findings carried forward unchanged.
- **1 new INFO finding** added through deeper analysis of Anthropic client usage:
  - **INFO-50**: Synchronous `anthropic.Anthropic()` client used in async functions `vision.py:79` and `xwines_matcher.py:153` blocks the event loop for up to 60 seconds per API call. Two other call sites (`drinkability_enrichment.py:179`, `import_service/mapping.py:191`) already use the correct `AsyncAnthropic` pattern. Converting to async is straightforward and eliminates a potential availability issue under concurrent load.
