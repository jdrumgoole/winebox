...
**Notable context:** No application code changes since the last review (2026-07-09). HEAD is at `45c83f1` (version 0.7.86). Dependency versions unchanged in lockfile. All 38 INFO-level findings from the previous review carried forward. INFO-38 updated with two additional Pillow CVEs (CVE-2026-55379, CVE-2026-55798) disclosed since the last review.
...
**INFO-38: Pillow 12.2.0 is affected by CVE-2026-54060, CVE-2026-55380, CVE-2026-55379, and CVE-2026-55798 — no practical impact on WineBox** :updated:
...
- **CVEs:**
  ...
  - **CVE-2026-55379** (CVSS 7.5) — `BdfFontFile._load()` reads BDF font files without calling `_decompression_bomb_check()`, allowing a crafted `.bdf` file to trigger excessive memory allocation. :new:
  - **CVE-2026-55798** (CVSS 4.5) — `WindowsViewer.get_command()` does not escape file paths, enabling OS command injection on Windows. :new:
- **Impact on WineBox: None.** WineBox processes only user-uploaded wine label photos. The image pipeline validates uploads via magic byte detection and only accepts JPEG, PNG, GIF, and WebP formats — GD format files and font files are rejected at upload. CVE-2026-55798 only affects Windows; WineBox runs on Linux. Neither vulnerable code path is reachable through WineBox's image handling.
...
- **Status:** Updated from 2026-07-09 review; CVE-2026-55379 and CVE-2026-55798 added 2026-07-10.
...
| Item | 2026-07-09 | 2026-07-10 | Status |
|------|-----------|-----------|--------|
...
- **1 existing finding updated** (INFO-38): added CVE-2026-55379 (BDF font file decompression bomb bypass, CVSS 7.5) and CVE-2026-55798 (Windows command injection, CVSS 4.5) to the existing Pillow entry. Same assessment: not exploitable via WineBox — font files are rejected at upload and the app runs on Linux.
- Full re-audit of all 12 categories confirms no regressions.

### Top 3 Priorities

1. **Batch dependency upgrade (INFO-33, INFO-34, INFO-26, INFO-38)** — cryptography (GHSA-537c-gmf6-5ccf), python-multipart (3 CVEs), PyJWT (5 CVEs), and Pillow (4 CVEs) are the highest-value upgrades. All are straightforward version bumps with no expected breaking changes. This single maintenance session would close 15 CVEs.
