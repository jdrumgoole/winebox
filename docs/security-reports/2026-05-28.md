...
**Notable context:** No application code changes since the last review (2026-05-27). HEAD is at commit `d7d5d1e` (the 2026-05-27 security review merge). Dependency CVE databases re-checked; no new advisories since yesterday. Today's audit conducted a deeper pass on input validation field constraints, rate limiting coverage on write endpoints, and cascading delete scoping.
...
**INFO-9: Cascading delete in `delete_wine` does not scope by `owner_id`** *(New)*
...
**INFO-10: Admin `delete_user` does not clean up `CellarEvent` or `CellarItem` documents** *(New)*
...
**INFO-11: Several Pydantic fields missing `max_length` constraints** *(New)*
...
**INFO-12: Vision-calling write endpoints rely on global rate limit only** *(New)*
...
- Only one commit since last review: `d7d5d1e` (merge of 2026-05-27 security review — documentation only).
- No security-concerning changes in the last 30 commits (daily security reviews, version bumps, price tracker XSS fix).
...
| :yellow_circle: INFO | 12 |
| :green_circle: CLEAN | 12 categories |

**Date of review:** 2026-05-28
**Reviewer:** Automated security audit (Claude Code)
**Codebase version:** 0.7.85 (commit d7d5d1e)

### Comparison with Previous Review (2026-05-27)

| Item | 2026-05-27 | 2026-05-28 | Status |
|------|-----------|-----------|--------|
...
| INFO-8 (Starlette CVE-2026-48710 BadHost) | New | Carried forward | No change |
| INFO-9 (delete_wine cascade not scoped) | — | New | Defence-in-depth |
| INFO-10 (delete_user orphaned data) | — | New | Data hygiene |
| INFO-11 (missing max_length constraints) | — | New | Defence-in-depth |
| INFO-12 (vision endpoints global rate limit) | — | New | Rate limit coverage |

### Changes Since Last Review

- No code changes since last review. All dependency versions unchanged.
- Today's audit conducted a deeper pass on cascading delete scoping, Pydantic field constraints, and rate limit coverage on write endpoints, surfacing 4 new INFO-level defence-in-depth findings (INFO-9 through INFO-12). None represent exploitable vulnerabilities.
