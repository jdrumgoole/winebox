...
**Notable context:** Since the last review (2026-05-17), the only code changes are the price tracker photo extension fix (`8150080`), a version bump to 0.7.85, and the regstack floor bump to 0.7.0. The previous WARNING-1 has been resolved.
...
None. The previous WARNING-1 (price tracker photo extension using untrusted client-supplied filename) was fixed in commit `8150080`. The fix uses magic-byte detection to derive the on-disk extension, consistent with the established `ImageStorageService.detect_image_type()` pattern. Regression test added.
...
- **Status:** Carried forward from 2026-05-17 review (INFO-5).
...
- **Status:** Carried forward from 2026-05-17 review (INFO-6).
...
- **Status:** Carried forward from 2026-05-17 review (INFO-7).
...
| regstack | >=0.7.0 | 0.7.0 | No known CVEs; upgrade from 0.6.0 |

No dependencies are yanked, compromised, or missing critical patches. `python-multipart` upgraded from 0.0.26 to 0.0.28 in lockfile since last review.
...
- Last 30 commits: CI fixes, version bumps, security reviews, and the price tracker fix. No security-concerning changes.
- Only code changes since last review: price tracker magic-byte fix (`8150080`), version bump to 0.7.85 (`dbfed40`), regstack floor to 0.7.0 (`a5464ff`).
...
**Codebase version:** 0.7.85 (commit dbfed40)

### Comparison with Previous Review (2026-05-17)

| Item | 2026-05-17 | 2026-05-18 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 1 | 0 | **Resolved** |
| INFO | 7 | 7 | Carried forward |
| WARNING-1 (price tracker photo extension) | Open | **Fixed** (commit `8150080`) | Resolved |
...
- **WARNING-1 resolved:** Price tracker photo upload now uses magic-byte detection to derive the on-disk extension (commit `8150080`), eliminating the stored-XSS vector from untrusted file extensions. Regression test added.
- **regstack upgraded:** Floor bumped from 0.6.0 to 0.7.0 (commit `a5464ff`).
- **python-multipart upgraded:** Lockfile updated from 0.0.26 to 0.0.28.
