...
**Notable context:** No application code changes since the last review (2026-06-21). HEAD is at `c5a0adf` (version 0.7.86). Dependency versions unchanged. All 27 INFO-level findings from the previous review carried forward. Five new INFO-level findings discovered through deeper analysis of price-tracker.js client-side escaping, import processor error handling, custom field name injection, console email token logging, and price photo deletion path safety (INFO-28 through INFO-32).
...
**INFO-28: price-tracker.js has three unescaped values in HTML string construction** *(new)*
...
- **Status:** New finding. Discovered through deeper client-side escaping audit.
...
- **Status:** New finding. Distinct from INFO-2 (specific exception types vs generic `Exception`).
...
- No code changes since last review. HEAD is at `c5a0adf` (Security Review — 2026-06-21 — security-clean).
...
**Date of review:** 2026-06-22
**Reviewer:** Automated security audit (Claude Code)
**Codebase version:** 0.7.86 (commit c5a0adf)

### Comparison with Previous Review (2026-06-21)

| Item | 2026-06-21 | 2026-06-22 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 0 | Maintained |
| INFO | 27 | 32 | +5 new findings from deeper audit |

### Changes Since Last Review

- No application code changes since last review (HEAD at `c5a0adf`).
- All dependency versions unchanged.
- **INFO-28 (new):** price-tracker.js has three unescaped values in HTML string construction — `photo_url` in img src attribute, `formatPrice` catch-path currency, and `c.id` in data attribute. All mitigated by CSP and server-controlled data sources.
- **INFO-29 (new):** Import processor broad `except Exception` returns raw `str(e)` to client, unlike INFO-2 which covers specific exception types with controlled messages. Could leak internal details if infrastructure errors occur during import processing.
- **INFO-30 (new):** Custom field name dot injection in import remap — a field name containing `.` creates nested MongoDB document structure within the user's own `custom_fields`. Cannot escape the prefix or affect other users.
- **INFO-31 (new):** Console email backends log full email body including verification/reset tokens at INFO level. Only affects development/testing (production uses SES).
- **INFO-32 (new):** Price photo delete endpoint constructs file path from database-stored `photo_path` without the path traversal checks used by the read endpoint. Data is always server-generated UUIDs.
- Full re-audit of all 12 categories confirms no regressions.
- No new CVEs discovered since last review.
