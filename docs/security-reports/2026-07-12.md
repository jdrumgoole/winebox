...
**Notable context:** No application code changes since the last review (2026-07-09). HEAD is at `45c83f1` (version 0.7.86). Dependency versions unchanged in lockfile. All 38 INFO-level findings from the previous review carried forward. INFO-38 updated with two additional Pillow CVEs (CVE-2026-55379, CVE-2026-55798). One new INFO finding (INFO-39) for inconsistent client-side `escapeHtml()` usage.
...
**INFO-38: Pillow 12.2.0 is affected by four CVEs (decompression bomb bypasses and OS command injection) — no practical impact on WineBox**
...
- **CVEs:**
  ...
  - **CVE-2026-55379** (HIGH) — Memory allocation without limits in BDF font file parsing. A crafted BDF file can trigger excessive memory allocation. :new:
  - **CVE-2026-55798** — OS command injection in `ImageShow` on Windows via crafted filenames. :new:
- **Impact on WineBox: None.** WineBox processes only user-uploaded wine label photos. The image pipeline validates uploads via magic byte detection and only accepts JPEG, PNG, GIF, and WebP formats — GD format and font files are rejected at upload. WineBox runs on Linux, so the Windows-specific CVE-2026-55798 is not applicable. Neither vulnerable code path is reachable through WineBox's image handling.
...
- **Status:** Carried forward from 2026-07-09 review; CVE-2026-55379 and CVE-2026-55798 added 2026-07-12.

**INFO-39: Inconsistent `escapeHtml()` usage in client-side JavaScript** :new:

- **Files and locations:**
  - `winebox/static/js/app.js:5354` — Wine type keys from uploaded CSV data interpolated into `innerHTML` without `escapeHtml()`. The `by_wine_type` summary keys come from user-controlled CSV content. Self-XSS only (user uploads their own CSV); mitigated by CSP (`script-src 'self'` blocks inline scripts).
  - `winebox/static/js/app.js:6296,6766` — `wine.front_label_image_path` interpolated into `<img src>` without `escapeHtml()`. Compare with lines 2816-2817 and 2963-2964 which correctly escape the same field. The path is always a server-generated UUID filename; exploitation requires database poisoning.
  - `winebox/static/js/app.js:4436,4532,4592` — `wine.wine_type` interpolated into CSS class attributes without escaping. Data comes from the curated X-Wines reference database.
  - `winebox/static/js/app.js:2510,2589,2722,6305,6358,6774` — `wine.vintage` not escaped in several views (compare with lines 2825, 2976, 3363 which DO escape it). Typically an integer; low risk.
- **Impact:** Very low. The Content-Security-Policy (`script-src 'self'` without `unsafe-inline`) blocks inline script execution, reducing these from XSS to HTML injection. Most data sources are server-controlled (UUIDs, curated reference data, validated integers). The CSV import path (line 5354) is the highest-risk vector but is self-XSS only.
- **Recommendation:** Apply `escapeHtml()` consistently to all server data interpolated into HTML, even when the data source is "trusted." The existing `escapeHtml()` in `app.js:3585-3596` handles all five critical characters.
- **Status:** New in 2026-07-12 review.
...
- Three dev/screenshot scripts contain hardcoded `TestPassword123!` — known since 2026-05-03, not production credentials.
...
| Item | 2026-07-09 | 2026-07-12 | Status |
|------|-----------|-----------|--------|
...
- All 38 INFO findings carried forward.
- **1 existing finding updated** (INFO-38): added CVE-2026-55379 (Pillow BDF font memory exhaustion) and CVE-2026-55798 (Pillow ImageShow OS command injection on Windows). Same assessment: not applicable to WineBox — only JPEG/PNG/GIF/WebP accepted, and WineBox runs on Linux.
- **1 new INFO finding** (INFO-39): Inconsistent `escapeHtml()` usage in client-side JavaScript — several locations in `app.js` interpolate server data into `innerHTML` without escaping. Mitigated by CSP (`script-src 'self'` blocks inline scripts). Highest-risk vector is self-XSS via crafted CSV import data.
