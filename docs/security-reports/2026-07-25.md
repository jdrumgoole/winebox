...
- **Impact on WineBox: None.** WineBox does not use path-based auth middleware. Authentication is enforced via FastAPI dependency injection (`RequireAuth`, `RequireAdmin`) on individual route handlers, which operate on the ASSI scope path (the correctly-routed path), not `request.url.path`. Additionally, nginx normalises Host headers in production.
...
- Only change since last review: yesterday's security report merged (commit `8512eb9`). HEAD at `8512eb9`.
...
**Codebase version:** 0.7.86 (commit 8512eb9)

### Comparison with Previous Review (2026-07-24)

| Item | 2026-07-24 | 2026-07-25 | Status |
|------|-----------|-----------|--------|
...
- No application code changes since last review (HEAD at `8512eb9`, previous security review commit only).
