...
**Notable context:** One infrastructure commit since the last review (2026-08-06): `60acc58` adds `invoke prod-ssl` for apex cert renewal and removes the dead `auto-merge-security-clean.yml` workflow. No application code changes. HEAD is at `60acc58` (version 0.7.86). Dependency versions unchanged in lockfile. All 56 INFO findings carried forward unchanged. Minor update to INFO-7 (cryptography latest now 50.0.0).
...
- **Status:** Carried forward from 2026-05-17 review. Updated 2026-08-08 — latest is now 50.0.0 (was 49.0.0+).
...
- **Recommendation:** Prioritize this upgrade alongside the existing INFO-7 version gap recommendation. The OpenSSL advisory adds urgency beyond the version-gap concern. See also INFO-51 for two additional CVEs requiring 49.0.0.
- **Status:** Carried forward from 2026-07-06 review. Updated 2026-08-04 with cross-reference to INFO-51.
...
- **CVEs:**
  - **CVE-2026-69248** (CVSS 3.1) — The X.509 verifier accepts wildcard DNS names (e.g. `*.example.com`) even when `permittedSubtrees` only allows a more specific name (e.g. `foo.example.com`), allowing certificate scope escape. CWE-295: Improper Certificate Validation.
  - **CVE-2026-69249** — Duplicate self-signed intermediate certificates in a chain cause exponential recursion in `build_chain_inner`, leading to resource exhaustion (>5s to reject in testing). Availability-only impact; validation correctness is not affected.
- **Impact on WineBox: None.** WineBox does not directly validate X.509 certificates with name constraints. Certificate chain validation is handled by the system TLS stack for outbound HTTPS connections to well-known services (MongoDB Atlas, Anthropic API, S3, PostHog). An attacker cannot present crafted certificate chains to WineBox's outbound connections.
- **Upgrade path:** Bump cryptography to >=49.0.0. This supersedes INFO-33's recommendation of >=48.0.1 — upgrading to 50.0.0 covers both the OpenSSL advisory and these two CVEs.
- **Recommendation:** The cumulative count of CVEs and advisories affecting cryptography 46.0.7 (INFO-7 version gap + INFO-33 OpenSSL advisory + these two CVEs) makes the upgrade to 50.0.0 the single highest-value dependency bump.
...
- **Detail:** When recording events (sold, gifted, consumed, etc.) on cases and bottles, the `CellarEvent` document is created with `cellar_id` but omits `owner_id` and `wine_id`. Compare with `cellar_debit.py:190-192` and `bottle_service.py:102-105` which correctly set both fields. The `cellar_event_view.py` query uses `$or: [{owner_id}, {cellar_id}]` fallback so queries still work.
- **Impact:** Data integrity issue — events without `wine_id` won't appear in wine transaction history, and won't be cleaned up when a wine is deleted (`crud.py:188`: `CellarEvent.find({"wine_id": wine.id}).delete()`). Not a data leak since `cellar_id` scoping prevents cross-user access. Related to INFO-9 and INFO-44 patterns.
- **Recommendation:** Add `owner_id=current_user.id` and `wine_id=item["wine"]["wine_id"]` to both event creation sites.
...
- **Detail:** The `winebox.app` landing page is served directly by nginx (not proxied to the FastAPI app), so the `SecurityHeadersMiddleware` CSP header is not applied. The nginx location block sets `X-Content-Type-Options` and `X-Frame-Options` but not `Content-Security-Policy`. The landing page is static marketing content with no user data or dynamic input.
- **Impact:** Very low — the landing page has no user data, forms, or dynamic content. No XSS attack surface.
- **Recommendation:** Add `add_header Content-Security-Policy "default-src 'self'; frame-ancestors 'none'; base-uri 'self'; object-src 'none';" always;` to the landing page server block for defence in depth.
- **Status:** Carried forward from 2026-08-05 review.

**INFO-54: Deprecated `datetime.utcnow()` usage in migration script and test**

- **Files:**
  - `scripts/migrations/migrate_sqlite_to_mongo.py` (lines 264-466) — Uses `datetime.utcnow()` in multiple locations for timestamp generation.
  - `tests/test_wine_ownership.py` (lines 39-53) — Uses `datetime.utcnow()` for test data timestamps.
- **Detail:** The deprecated `datetime.utcnow()` produces timezone-naive datetimes. The main application code correctly uses `datetime.now(timezone.utc)` everywhere. These occurrences are in an archived migration script and a test file only, not in production code.
- **Impact:** None — scripts/tests only, not production code paths.
- **Recommendation:** Update to `datetime.now(timezone.utc)` for consistency with the project's datetime policy (CLAUDE.md).
- **Status:** Carried forward from 2026-08-05 review.
...
- **Detail:** When a case or bottle event (sold, gifted, consumed) is recorded, the case/bottle item's quantity is correctly adjusted, but the parent `Wine.inventory.quantity` is not decremented. Compare with `add_cases()` at `cases.py:148-151` which correctly syncs `Wine.inventory.quantity` via `$inc`. This means cellar summary and search filters (which rely on `inventory.quantity`) show stale stock counts after events are recorded through these endpoints.
- **Impact:** Functional data consistency issue, not a security vulnerability. No data leak or access control bypass.
- **Recommendation:** Add `Wine.find_one_and_update({"_id": ..., "owner_id": current_user.id}, {"$inc": {"inventory.quantity": -quantity}})` to both event creation paths.
- **Status:** Carried forward from 2026-08-05 review.

**INFO-56: Unbounded `skip` parameter on most list endpoints**

- **Files:**
  - `winebox/routers/search.py:46` — `skip: int = Query(0, ge=0)` with no upper bound
  - `winebox/routers/wines/crud.py:23` — `skip: int = Query(0, ge=0)` with no upper bound
  - `winebox/routers/cellar.py:25` — `skip: int = Query(0, ge=0)` with no upper bound
  - `winebox/routers/transactions.py:32` — `skip: int = Query(0, ge=0)` with no upper bound
  - `winebox/routers/met.py:17` — `skip: int = Query(0, ge=0)` with no upper bound
  - `winebox/routers/price_tracker.py:307,380` — `skip: int = Query(0, ge=0)` with no upper bound
  - `winebox/admin/routers/admin.py:53` — `skip: int = Query(0, ge=0)` with no upper bound
- **Detail:** All list endpoints except X-Wines search (`xwines.py:482` has `le=10000`) accept `skip` with no maximum. A large `skip` value (e.g. `skip=999999999`) forces MongoDB to scan and discard many documents before returning results. All affected queries are owner-scoped (personal data sets are small) and rate-limited (global 60/min), and the MongoDB socket timeout (30s) provides a backstop. The X-Wines endpoint already demonstrates the correct pattern with `le=10000`.
- **Impact:** Very low — owner-scoped queries produce small result sets, rate limiting prevents sustained abuse, and MongoDB timeouts prevent indefinite blocking.
...
- One infrastructure commit since last review: `60acc58` — adds `invoke prod-ssl` task for apex cert renewal and removes the dead `auto-merge-security-clean.yml` workflow. Security-positive change (cert management hardening). No vulnerabilities introduced.
- No secret files accidentally committed.
- No force pushes, rebases, or reverts detected.
- Last application code change was commit `99e9586` (2026-06-12) — test-DB cleanup + Claude model swap. No security issues.
...
- New `invoke prod-ssl` task properly handles cert renewal with try/finally nginx restart and certbot renewal hooks for unattended auto-renew.
...
| Item | 2026-08-06 | 2026-08-08 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 0 | Maintained |
| INFO | 56 | 56 | No change |

### Changes Since Last Review

- One infrastructure commit since last review: `60acc58` — adds `invoke prod-ssl` task for apex cert renewal and removes the dead `auto-merge-security-clean.yml` GitHub Actions workflow. No application code changes. No new vulnerabilities introduced.
- All dependency versions unchanged in lockfile.
- Minor update to INFO-7: cryptography latest is now 50.0.0 (was 49.0.0+), widening the version gap to 4 major versions.
- All 56 INFO findings carried forward unchanged.
