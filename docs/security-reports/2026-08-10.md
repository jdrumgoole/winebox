...
**Notable context:** One commit since the last review (2026-08-06): `60acc58` — adds a `prod-ssl` invoke task for renewing the apex landing-page Let's Encrypt cert and removes the now-unused `auto-merge-security-clean.yml` GitHub Actions workflow. No application code, dependency, or configuration changes. HEAD at `60acc58` (version 0.7.86). All 56 INFO findings carried forward unchanged.
...
- **Status:** Carried forward from 2026-05-17 review. Updated 2026-08-04 with cross-reference to INFO-51.
...
- **Upgrade path:** Bump cryptography to >=49.0.0. This supersedes INFO-33's recommendation of >=48.0.1 — upgrading to 49.0.0 covers both the OpenSSL advisory and these two CVEs.
- **Recommendation:** The cumulative count of CVEs and advisories affecting cryptography 46.0.7 (INFO-7 version gap + INFO-33 OpenSSL advisory + these two CVEs) makes the upgrade to 49.0.0 the single highest-value dependency bump.
...
- One commit since last review: `60acc58` — adds `prod-ssl` invoke task for apex cert renewal and removes unused `auto-merge-security-clean.yml` GitHub Actions workflow. No application code, dependency, or configuration changes. HEAD at `60acc58`.
- The `prod-ssl` task follows the same security patterns as existing SSL tasks: uses `run_ssh()`, no hardcoded secrets, proper error handling with `finally` block to always restart nginx.
...
| Item | 2026-08-06 | 2026-08-10 | Status |
|------|-----------|-----------|--------|
...
- One commit since last review: `60acc58` — adds `prod-ssl` invoke task for apex Let's Encrypt cert renewal (winebox.app + www.winebox.app) and removes the `auto-merge-security-clean.yml` GitHub Actions workflow. The `prod-ssl` task was reviewed and found clean: follows established SSH patterns, no hardcoded secrets, proper error handling.
- All dependency versions unchanged in lockfile.
- No new findings — all 56 INFO findings carried forward unchanged.
