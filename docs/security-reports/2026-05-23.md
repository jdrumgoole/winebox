...
**Notable context:** No code changes since the last review (2026-05-22). HEAD is at commit `3d35ed0` (the 2026-05-22 security review merge). All findings are carried forward or newly verified against current dependency databases.
...
**INFO-8: Admin `CreateUserRequest` password field has no `max_length` constraint**

- **File:** `winebox/admin/routers/admin.py:193`
- **Detail:** The `password` field in `CreateUserRequest` is `str` with no `max_length`. The runtime check (line 213) enforces `min_length=8` but no upper bound. In contrast, regstack's registration uses `PasswordStr = Annotated[str, Field(min_length=8, max_length=128)]`. An extremely long password could cause Argon2id to consume excessive memory/CPU.
- **Impact:** Very low — endpoint requires `RequireAdmin`, is behind nginx IP allowlist, and nginx `client_max_body_size 10M` practically caps request body size.
- **Recommendation:** Add `max_length=128` to the password field for consistency with regstack.
- **Status:** Newly identified (present since admin panel was added; no code change triggered this).

**INFO-9: Admin logout endpoint uses `RequireAuth` instead of `RequireAdmin`**

- **File:** `winebox/admin/routers/auth.py:70`
- **Detail:** The admin `POST /api/auth/logout` uses `RequireAuth` instead of `RequireAdmin`. All other admin endpoints use `RequireAdmin`. A non-admin user with a valid token who was inside the IP allowlist could call this endpoint.
- **Impact:** None in practice — the admin login endpoint already rejects non-admins with 403, and the admin panel is IP-restricted at nginx.
- **Recommendation:** Change to `RequireAdmin` for consistency.
- **Status:** Newly identified (present since admin panel was added; no code change triggered this).

**INFO-10: `skip` query parameters lack upper bound on most list endpoints**

- **Files:** `winebox/routers/search.py:46`, `cellar.py:25`, `transactions.py:32`, `met.py:17`, `wines/crud.py:23`, `price_tracker.py:307,380`
- **Detail:** The `skip` parameter uses `Query(0, ge=0)` without a `le=` constraint on 7 endpoints. The X-Wines router correctly caps skip at `le=10000`. MongoDB `skip()` is O(n) on matching documents, but all queries are owner-scoped so the cost is bounded by the user's own data size (capped at `MAX_USER_RESULTSET = 10,000`).
- **Impact:** Negligible — a skip value exceeding the user's document count simply returns an empty result.
- **Recommendation:** Add `le=MAX_USER_RESULTSET` for consistency with the X-Wines router.
- **Status:** Newly identified (present since these endpoints were added; no code change triggered this).

**INFO-11: Unbounded `to_list()` in wine transaction history**

- **File:** `winebox/services/cellar_event_view.py:242`
- **Detail:** The wine transaction history query uses `.to_list()` without a `length` parameter. If a wine has an extremely large transaction history, this loads it all into memory. Similarly, `winebox/routers/bottles.py:190` has an unbounded `to_list()` for item events.
- **Impact:** Very low — queries are per-wine and owner-scoped, so the result set is naturally bounded by the user's activity on a single wine.
- **Recommendation:** Add `to_list(length=MAX_USER_RESULTSET)` for defence in depth.
- **Status:** Newly identified (present since these features were added; no code change triggered this).
...
| jinja2 | >=3.1.6 | 3.1.6 | No known 2026 CVEs (CVE-2026-5760 affects SGLang, not Jinja2 itself) |
...
- Only one commit since last review: `3d35ed0` (merge of 2026-05-22 security review — documentation only).
...
| INFO-8 (admin password max_length) | — | Newly identified | Defence-in-depth |
| INFO-9 (admin logout RequireAuth) | — | Newly identified | Defence-in-depth |
| INFO-10 (unbounded skip params) | — | Newly identified | Defence-in-depth |
| INFO-11 (unbounded to_list) | — | Newly identified | Defence-in-depth |
...
- 4 new INFO items identified through deeper defence-in-depth analysis (INFO-8 through INFO-11). All are pre-existing conditions with no exploitable risk due to existing controls (RequireAdmin, IP allowlist, owner-scoped queries).

### Top 3 Priorities

1. **Remove `cairosvg` if unused (INFO-5)** — reduces attack surface with no functionality loss.
2. **Add `max_length=128` to admin `CreateUserRequest.password` (INFO-8)** — consistency with regstack's password validation.
