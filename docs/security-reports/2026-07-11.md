...
**Notable context:** No application code changes since the last review (2026-07-09). HEAD is at `45c83f1` (version 0.7.86). Dependency versions unchanged in lockfile. All 38 INFO-level findings from the previous review carried forward. INFO-38 updated with two additional Pillow CVEs (CVE-2026-55379, CVE-2026-55798). INFO-34 updated with one additional python-multipart CVE (CVE-2026-53537).
...
- **Impact on WineBox: Limited.** Vulnerable parser is only invoked for endpoints with `Form()` parameters. All form endpoints require `RequireAuth`, and nginx caps payload at 10 MB. See also INFO-34 for three additional python-multipart CVEs.
...
**INFO-34: python-multipart 0.0.28 is affected by three additional CVEs (CVE-2026-53540, CVE-2026-53538, CVE-2026-53537)**
...
- **CVEs:**
  ...
  - **CVE-2026-53537** (Content-Disposition parameter smuggling via RFC 2231/5987 extended parameters, CVSS 3.1) — The `parse_options_header` function applies RFC 2231/5987 extended parameters transparently, allowing an attacker to smuggle a different field name or filename past an upstream inspector (WAF, proxy) to the backend. **Limited impact on WineBox:** WineBox does not use a WAF or reverse proxy that inspects Content-Disposition headers for security decisions. All file upload endpoints validate content independently. :new:
- **Upgrade path:** Bump `python-multipart>=0.0.31` (same as INFO-26 recommendation).
- **Recommendation:** These three CVEs add urgency to the python-multipart upgrade already recommended in INFO-26. All four CVEs are fixed in 0.0.31.
- **Status:** Carried forward from 2026-07-06 review; CVE-2026-53537 added 2026-07-11.
...
**INFO-38: Pillow 12.2.0 is affected by CVE-2026-54060 + CVE-2026-55380 + CVE-2026-55379 + CVE-2026-55798 (decompression bomb bypasses and OS command injection) — no practical impact on WineBox**
...
- **CVEs:**
  ...
  - **CVE-2026-55379** (CVSS 7.5) — `BdfFontFile.bdf_char()` reads BBX width and height from a BDF font file and passes attacker-controlled dimensions to `Image.new()` without calling `_decompression_bomb_check()`, bypassing decompression bomb protection and allowing excessive memory allocation. :new:
  - **CVE-2026-55798** (CVSS 4.5) — `WindowsViewer.get_command()` constructs a `cmd.exe` shell command by directly embedding a file path into an f-string without escaping, allowing shell metacharacters in the file path to inject arbitrary commands. Requires user interaction on Windows. :new:
- **Impact on WineBox: None.** WineBox processes only user-uploaded wine label photos. The image pipeline validates uploads via magic byte detection and only accepts JPEG, PNG, GIF, and WebP formats — GD format and BDF font files are rejected at upload. WineBox does not process font files at any point. CVE-2026-55798 affects Windows only; WineBox runs on Linux. None of the four vulnerable code paths are reachable through WineBox's image handling.
...
- **Status:** Carried forward from 2026-07-09 review; CVE-2026-55379 and CVE-2026-55798 added 2026-07-11.
...
| python-multipart | >=0.0.26 | 0.0.28 | 4 CVEs — limited impact (see INFO-26, INFO-34) |
...
| Item | 2026-07-09 | 2026-07-11 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 0 | Maintained |
| INFO | 38 | 38 | Unchanged |
...
- **2 existing findings updated** with newly disclosed CVEs:
  - INFO-38: Added CVE-2026-55379 (Pillow BdfFontFile decompression bomb bypass, CVSS 7.5) and CVE-2026-55798 (Pillow WindowsViewer OS command injection, CVSS 4.5). Neither is exploitable via WineBox — BDF font files are rejected at upload and WineBox runs on Linux, not Windows.
  - INFO-34: Added CVE-2026-53537 (python-multipart Content-Disposition parameter smuggling via RFC 2231/5987, CVSS 3.1). Limited impact — WineBox does not use a WAF that inspects Content-Disposition headers.
- Full re-audit of all 12 categories confirms no regressions.

### Top 3 Priorities

1. **Batch dependency upgrade (INFO-33, INFO-34, INFO-26, INFO-38)** — cryptography (GHSA-537c-gmf6-5ccf), python-multipart (4 CVEs), PyJWT (5 CVEs), and Pillow (4 CVEs) are the highest-value upgrades. All are straightforward version bumps with no expected breaking changes. This single maintenance session would close 14 CVEs.
