...
**Notable context:** No application code changes since the last review (2026-06-05). HEAD is at commit `dc654b3` (the 2026-06-05 security review merge). Dependency pins and lockfile are unchanged. Two new CVEs affecting locked dependencies (PyJWT, aiohttp) were identified in the 2026-06-06 review (PR #97, still open) and are confirmed as WARNING-level findings today.
...
**WARNING-1: CVE-2026-48526 — PyJWT algorithm confusion vulnerability**

- **File:** `uv.lock` — pyjwt pinned at 2.12.1; fix is in pyjwt 2.13.0
- **CVE:** CVE-2026-48526 — Algorithm confusion allows an attacker who obtains the public JWK to sign forged tokens using it as an HMAC secret, bypassing authentication.
- **Impact on WineBox: Not exploitable.** WineBox uses HS256 exclusively with a shared secret key (`WINEBOX_REGSTACK_JWT_SECRET`). There is no JWK endpoint, no public key exposure, and no mixed-algorithm configuration. The algorithm is hardcoded in regstack's config, not user-controllable. The vulnerability requires a JWK/RSA setup to exploit.
- **Fix:** Bump `pyjwt>=2.13.0` in `pyproject.toml` + `uv lock --upgrade-package pyjwt` for defence in depth.
- **Status:** First identified in 2026-06-06 review (PR #97, open).

**WARNING-2: CVE-2026-34993 + CVE-2026-47265 — aiohttp pickle RCE and cookie leak**

- **File:** `uv.lock` — aiohttp pinned at 3.13.5; fixes are in aiohttp 3.14.0
- **CVEs:**
  - CVE-2026-34993 — Unsafe deserialization in `CookieJar.load()` allows arbitrary code execution.
  - CVE-2026-47265 — Cross-origin redirect cookie leak exposes session cookies to third-party domains.
- **Impact on WineBox: Not exploitable.** aiohttp is a transitive dependency (via aioboto3 → botocore) with no direct usage. WineBox never calls `CookieJar.load()`, never instantiates aiohttp sessions directly, and never makes aiohttp-mediated HTTP requests with cookies. The botocore usage is server-side AWS API calls with IAM credentials, not cookie-based sessions.
- **Fix:** Bump `aiohttp>=3.14.0` in `pyproject.toml` + `uv lock --upgrade-package aiohttp` for defence in depth.
- **Status:** First identified in 2026-06-06 review (PR #97, open).
...
- **Detail:** The admin delete-user endpoint removes wines, transactions, import batches, and raw uploads, but does not remove `CellarEvent` or `CellarItem` documents for the deleted user. Additionally, the `raw_uploads` delete queries by `owner_id` but `RawUploadRow` has no `owner_id` field (only `batch_id`), making that delete a no-op. This leaves orphaned data in the database. No security risk — the orphaned documents are scoped by `cellar_id`/`owner_id` and cannot be accessed by other users.
- **Recommendation:** Add `CellarEvent` and `CellarItem` cleanup to the delete cascade. Fix `raw_uploads` deletion to query by `{"batch_id": {"$in": batch_ids}}` after collecting batch IDs.
- **Status:** Carried forward from 2026-05-28 review (refined with raw_uploads no-op detail).
...
- **Detail:** Both queries are scoped to a single wine/item and naturally produce small result sets. However, adding `length=MAX_USER_RESULTSET` would provide defence-in-depth against any future data shape that could inflate event counts.
- **Impact:** Very low — event counts per item are naturally small.
...
**INFO-18: CVE-2026-44431 — urllib3 header leak on cross-origin redirects (not exploitable)**

- **File:** `uv.lock` — urllib3 pinned at 2.6.3; fix is in urllib3 2.7.0
- **CVE:** CVE-2026-44431 — Sensitive headers (`Authorization`, `Cookie`) are not stripped during cross-origin redirects when using urllib3's low-level `ProxyManager` API.
- **Impact on WineBox: None.** WineBox does not use urllib3 directly. It is a transitive dependency via `requests` and `botocore`. Neither WineBox nor its direct dependencies use `ProxyManager`. All HTTP client usage goes through `httpx` (for application HTTP calls) and `aioboto3`/`botocore` (for AWS API calls, which use IAM signature authentication, not header-forwarded credentials).
- **Recommendation:** Consider pinning `urllib3>=2.7.0` as a transitive dependency floor in a future maintenance cycle for defence in depth.
- **Status:** New finding.

**INFO-19: regstack account management routes rely on global rate limit only**

- **File:** `winebox/auth/regstack_setup.py:45-53` (missing entries for `change_email`, `confirm_email_change`, `delete_account`)
- **Detail:** The `_RATE_LIMITS` dict configures per-route rate limits for login, register, forgot-password, reset-password, verify, resend-verification, and change-password. However, regstack's account router also exposes `POST /api/auth/change-email` (sends verification email to new address), `POST /api/auth/confirm-email-change`, and `DELETE /api/auth/account`. These routes fall back to the global 60/minute rate limit.
- **Mitigating factors:**
  - `change_email` requires the user's current password, limiting abuse to the account holder.
  - Email sending is rate-limited by SES at the infrastructure layer.
  - `delete_account` also requires the current password.
  - `confirm_email_change` consumes a one-time token (self-limiting).
  - regstack 0.7.0 does not expose configurable rate limit fields for these routes, so WineBox cannot set them via `_RATE_LIMITS`.
- **Impact:** Low — password requirement prevents third-party abuse; SES provides a backstop for email sending.
- **Recommendation:** Request rate limit configuration fields for account management routes in a future regstack release, or override these routes in WineBox to add explicit rate limits.
- **Status:** New finding.
...
All key dependencies are at patched versions (except PyJWT and aiohttp — see WARNING-1 and WARNING-2):
...
| pyjwt | >=2.12.0 | 2.12.1 | **CVE-2026-48526 — see WARNING-1** |
...
| aiohttp | >=3.13.4 | 3.13.5 | **CVE-2026-34993 + CVE-2026-47265 — see WARNING-2** |
...
| urllib3 | — | 2.6.3 | CVE-2026-44431 — no practical impact (see INFO-18) |

No dependencies are yanked or compromised. No lockfile changes since last review. No supply chain concerns — none of the 2026 compromised packages (LiteLLM, PyTorch Lightning, durabletask) are WineBox dependencies.
...
- Anti-enumeration: forgot-password and change-email always return the same message regardless of email existence.
...
- No `eval()`, `exec()`, or `__import__()` in application code. Only `ast.literal_eval()` on internal database fields (X-Wines grapes data).
...
- Bearer-only token transport (no cookies) — cookie security flags not applicable.
...
- Only commit since last review: `dc654b3` (merge of 2026-06-05 security review — documentation only).
...
- CI pipeline healthy: all recent workflow runs succeeded.
- Open PR #97 (2026-06-06 security-warning) contains the same CVE findings as WARNING-1 and WARNING-2 above.
- Open PR #93 (2026-06-03 security-warning) contains additional findings, some of which informed INFO-19 above.
...
| :yellow_circle: INFO | 19 |
| :green_circle: CLEAN | 12 categories |

**Date of review:** 2026-06-07
**Reviewer:** Automated security audit (Claude Code)
**Codebase version:** 0.7.85 (commit dc654b3)

### Comparison with Previous Review (2026-06-05)

| Item | 2026-06-05 | 2026-06-07 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 2 | **+2 new CVEs** |
| INFO | 17 | 19 | +2 new |
| WARNING-1 (PyJWT CVE-2026-48526) | — | New | First identified 2026-06-06 |
| WARNING-2 (aiohttp CVE-2026-34993/47265) | — | New | First identified 2026-06-06 |
| INFO-1 through INFO-17 | Carried forward | Carried forward | No change |
| INFO-10 (delete_user orphaned data) | Carried forward | Refined | Added raw_uploads no-op detail |
| INFO-18 (urllib3 CVE-2026-44431) | — | New | Not exploitable |
| INFO-19 (regstack account mgmt rate limits) | — | New | Low impact |

### Changes Since Last Review

- No code changes since last review. All dependency versions unchanged.
- 17 INFO-level findings carried forward unchanged; 2 new INFO findings added.
- 2 new WARNING-level CVEs confirmed (PyJWT CVE-2026-48526, aiohttp CVE-2026-34993 + CVE-2026-47265) — neither is exploitable in WineBox's current architecture, but version bumps are recommended for defence in depth.
- PR #97 (2026-06-06) is still open and addresses the same WARNING findings with a proposed fix (bump pyjwt>=2.13.0 and aiohttp>=3.14.0).
...
1. **Merge PR #97 or bump PyJWT and aiohttp** (WARNING-1, WARNING-2) — while neither CVE is exploitable in WineBox today, bumping to `pyjwt>=2.13.0` and `aiohttp>=3.14.0` removes the theoretical risk. The fix is a one-line pin change plus `uv lock --upgrade-package`.
2. **Plan coordinated FastAPI + Starlette upgrade (INFO-8)** — upgrading to FastAPI >=0.136 + Starlette >=1.0.1 removes the CVE-2026-48710 theoretical risk and keeps the stack current.
3. **Remove `cairosvg` if unused (INFO-5)** — reduces attack surface with no functionality loss.
