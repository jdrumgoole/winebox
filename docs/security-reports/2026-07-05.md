...
**Notable context:** No application code changes since the last review (2026-07-04). HEAD is at `8b2264c` (version 0.7.86). Dependency versions unchanged. One new INFO finding discovered (INFO-33). All 32 prior INFO findings carried forward. No new CVEs affecting WineBox.
...
**INFO-33: Transaction export wine-details lookup does not scope by `owner_id`** *(NEW)*
...
- **Code:**
  ...
  ```
- **Detail:** When exporting transactions with `include_wine_details=true`, the endpoint batch-fetches wine documents by their IDs without adding an `owner_id` filter. The wine IDs are extracted from CellarEvent documents already scoped to the current user via `build_event_query(current_user.id, ...)` at line 209. MongoDB ObjectIds are globally unique, so there is zero practical risk of cross-user data leakage. However, adding `"owner_id": current_user.id` to the filter would provide defence-in-depth consistent with the project's security guidelines.
- **Impact:** None — the wine IDs are derived from owner-scoped events, and ObjectIds are unpredictable.
- **Recommendation:** Add `"owner_id": current_user.id` to the Wine query filter for consistency with the codebase's defence-in-depth patterns.
- **Status:** New finding. Same pattern as INFO-9.
...
- **All database queries** for user-owned data filter by `owner_id` or `cellar_id` (one exception noted in INFO-33).
...
- No code changes since last review. HEAD is at `8b2264c` (previous security review commit only).
...
| Item | 2026-07-04 | 2026-07-05 | Status |
|------|-----------|-----------|--------|
...
- One new INFO finding (INFO-33): transaction export wine-details lookup missing `owner_id` filter — zero practical risk, defence-in-depth gap only.
- All 32 prior INFO findings carried forward unchanged.
- Full re-audit of all 12 categories confirms no regressions.
- No new CVEs discovered affecting WineBox since last review.
...
3. **Add `owner_id` filter to export wine lookup (INFO-33)** — one-line fix that brings the transaction export in line with the project's defence-in-depth standard. Same pattern as INFO-9.
