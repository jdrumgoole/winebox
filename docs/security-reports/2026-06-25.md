...
**Notable context:** No application code changes since the last review (2026-06-24). HEAD is at `3b6c698` (version 0.7.86). Dependency versions unchanged. All 32 INFO-level findings from the previous review carried forward. No new findings.
...
**INFO-19: aiohttp 3.13.5 is affected by multiple CVEs (3.14.0 + 3.14.1) — no practical impact on WineBox**

- **File:** `uv.lock` — aiohttp pinned at 3.13.5; patches are in aiohttp 3.14.0 and 3.14.1
- **CVEs in aiohttp 3.14.0 (not covered by locked version):**
  - **CVE-2026-47265** — Cross-origin redirect cookie leak. CVSS 6.6 (Medium). WineBox does not pass per-request cookies via aiohttp.
  - **CVE-2026-34993** — Arbitrary code execution via `CookieJar.load()`. WineBox does not use `CookieJar.load()` (see INFO-21).
  - **CVE-2026-50269** — CRLF injection in multipart headers. Low (CVSS 2.7). WineBox does not construct multipart requests with user-controlled headers via aiohttp.
- **CVEs in aiohttp 3.14.1 (not covered by locked version):**
  - **CVE-2026-54273** — HTTP/1 pipelined requests queue without limit (DoS). Server-side only; WineBox uses aiohttp as a client.
  - **CVE-2026-54274** — Incomplete websocket frames bypass memory limits. WineBox does not use aiohttp websockets.
  - **CVE-2026-54275** — TLS hostname override ignored on connection reuse. The anthropic SDK connects to a single host (api.anthropic.com), so reuse is same-origin.
  - **CVE-2026-54276** — DigestAuthMiddleware applies credentials cross-origin. WineBox does not use DigestAuth.
  - **CVE-2026-54277** — C parser bypasses `max_line_size` for fragmented lines. Client-side impact limited; the anthropic SDK connects to trusted servers.
  - **CVE-2026-54278** — Compressed request bodies bypass `client_max_size`. Server-side only.
  - **CVE-2026-54279** — Host-only cookies become domain cookies after CookieJar persistence. WineBox does not persist CookieJars.
  - **CVE-2026-54280** — Payload response resources not closed after disconnect. Minor resource leak, no security impact.
- **Impact on WineBox: None.** aiohttp is a transitive dependency (pulled in by the `anthropic` SDK). WineBox never imports or uses aiohttp directly. None of the affected features (CookieJar persistence, DigestAuth, websockets, server-side pipelining) are used.
- **Upgrade path:** Bump aiohttp to >=3.14.1. Transitive dependency; updating may require waiting for the `anthropic` SDK to update its pin.
- **Recommendation:** Monitor for an `anthropic` SDK release that pulls in aiohttp >=3.14.1. None of the vulnerabilities are exploitable via WineBox's usage pattern.
- **Status:** CVE-2026-47265 carried forward from 2026-06-08; CVE-2026-54273 through CVE-2026-54280 newly identified in this review.
...
**INFO-26: python-multipart 0.0.28 is affected by three CVEs fixed in 0.0.30 — limited impact on WineBox**

- **File:** `uv.lock` — python-multipart pinned at 0.0.28; patches are in python-multipart 0.0.30
- **CVEs (all fixed in 0.0.30):**
  - **CVE-2026-53539** — Quadratic-time querystring parsing with semicolons causes CPU DoS (O(n^2) byte comparisons). **Limited impact:** form-accepting endpoints require `RequireAuth`, nginx caps body size at 10 MB, and global rate limit (60/min) provides baseline protection. A single well-crafted 10 MB form body could still cause significant CPU load on one worker.
  - **CVE-2026-53537** — Content-Disposition parameter smuggling via RFC 2231/5987 extended parameters allows filename spoofing. **No practical impact:** WineBox validates file type via magic bytes (not filename), so filename spoofing cannot bypass upload security.
  - **CVE-2026-53538** — Semicolons treated as querystring field separators, enabling parameter injection. **No practical impact:** FastAPI uses Pydantic models for all inputs with explicit field definitions, limiting the scope of parameter injection.
- **Upgrade path:** Bump `python-multipart>=0.0.30` in pyproject.toml. No breaking changes expected.
- **Recommendation:** Upgrade to python-multipart >=0.0.30 in the next maintenance cycle. CVE-2026-53539 has limited real-world impact; the other two have no practical impact. The fix is straightforward.
- **Status:** CVE-2026-53539 carried forward from 2026-06-21; CVE-2026-53537 and CVE-2026-53538 newly identified in this review.
...
| python-multipart | >=0.0.26 | 0.0.28 | Patched for CVE-2026-40347 + CVE-2026-24486; three CVEs in 0.0.30 have limited/no impact (see INFO-26) |
...
| aiohttp | >=3.13.4 | 3.13.5 | Patched for CVE-2026-34520 + CVE-2026-34519 + CVE-2026-22815; 11 additional CVEs in 3.14.0/3.14.1 not exploitable (see INFO-19/21) |
...
- XSS prevention consistent across all HTML rendering: `app.js`, `toast.js`, static site template all define and use proper `escapeHtml()` with all five HTML metacharacters.
...
- No code changes since last review. HEAD is at `3b6c698` (Security Review — 2026-06-24 — security-clean).
...
**Codebase version:** 0.7.86 (commit 3b6c698)

### Comparison with Previous Review (2026-06-24)

| Item | 2026-06-24 | 2026-06-25 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 0 | Maintained |
| INFO | 32 | 32 | Unchanged |

### Changes Since Last Review

- No application code changes since last review (HEAD at `3b6c698`, previous security review commit only).
- All dependency versions unchanged.
- All 32 INFO findings carried forward — no new INFO items added, no resolved findings.
- **INFO-19 expanded:** 9 additional aiohttp CVEs (CVE-2026-50269, CVE-2026-54273 through CVE-2026-54280) newly disclosed in aiohttp 3.14.0/3.14.1. None exploitable in WineBox (transitive dependency, affected features not used).
- **INFO-26 expanded:** 2 additional python-multipart CVEs (CVE-2026-53537, CVE-2026-53538) identified in 0.0.30. No practical impact on WineBox (magic byte validation, Pydantic input models).
- Full re-audit of all 12 categories confirms no regressions. Overall severity unchanged.
...
3. **Fix price-tracker.js escaping gaps (INFO-28)** — upgrade the price-tracker's `escapeHtml()` to match the main app's 5-character implementation and use DOM APIs for attribute insertion. Low risk today, but easy to fix and brings the price tracker in line with the main app's strong escaping discipline.
