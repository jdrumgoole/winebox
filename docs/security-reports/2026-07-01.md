...
**Notable context:** No application code changes since the last review (2026-06-30). HEAD is at `90801e4` (version 0.7.86). Dependency versions unchanged. Two new CVE clusters discovered for python-multipart and aiohttp — neither is exploitable in WineBox. 32 INFO findings carried forward; 1 new INFO finding added (INFO-33). Total: 33 INFO findings.
...
**INFO-26: python-multipart 0.0.28 is affected by three CVEs — limited impact on WineBox**

- **File:** `uv.lock` — python-multipart pinned at 0.0.28; all three patched in python-multipart 0.0.30
- **CVEs affecting python-multipart 0.0.28:**
  - **CVE-2026-53539** (QuerystringParser DoS) — The `QuerystringParser` uses an inefficient algorithm to locate field separators in `application/x-www-form-urlencoded` POST data. Repeated semicolons cause O(B^2) byte comparisons, causing CPU denial of service.
  - **CVE-2026-53537** (Content-Disposition parameter smuggling) — RFC 2231/5987 extended parameter handling allows attackers to smuggle Content-Disposition parameters, potentially bypassing filename or field-name validation in multipart form uploads.
  - **CVE-2026-53538** (Semicolon querystring separator) — Semicolons treated as querystring field separators enable parameter smuggling, allowing injection of additional form fields.
- **Impact on WineBox: Limited.** The vulnerable parser is only invoked for endpoints that declare `Form()` parameters (record, met, checkout, scan, price capture). All form-accepting endpoints require `RequireAuth`, so an attacker must be authenticated. Additionally, nginx's `client_max_body_size 10M` caps the payload size, and the global rate limit (60/minute) provides baseline protection. File uploads validate content by magic bytes (not just filename), so filename smuggling via CVE-2026-53537 cannot bypass type validation. Parameter smuggling via CVE-2026-53538 is mitigated by Pydantic model validation on all form inputs.
- **Upgrade path:** Bump `python-multipart>=0.0.30` in pyproject.toml. No breaking changes expected.
- **Recommendation:** Upgrade to python-multipart >=0.0.30 in the next maintenance cycle. The risk is mitigated by authentication requirements, nginx body size limits, magic-byte file validation, and Pydantic input validation, but the fix is straightforward.
- **Status:** Updated from 2026-06-21 review — added CVE-2026-53537 and CVE-2026-53538 (new since 2026-06-30).
...
**INFO-33: aiohttp 3.13.5 is affected by CVE-2026-54274 and CVE-2026-54278 (websocket/compression bypasses) — no practical impact on WineBox**

- **File:** `uv.lock` — aiohttp pinned at 3.13.5; patches in aiohttp 3.14.1
- **CVEs:**
  - **CVE-2026-54274** — Incomplete websocket frame payloads bypass memory limits, allowing memory exhaustion via crafted websocket connections.
  - **CVE-2026-54278** — Compressed request bodies bypass `client_max_size` during cleanup, allowing oversized payloads to be processed.
- **Impact on WineBox: None.** aiohttp is a transitive dependency (pulled in by the `anthropic` SDK). WineBox does not use aiohttp as an HTTP server and does not expose aiohttp websocket endpoints. The Anthropic SDK uses aiohttp as an HTTP client only; neither CVE applies to client-side usage.
...
- **Recommendation:** Monitor for an `anthropic` SDK release that pulls in aiohttp >=3.14.1. Neither vulnerability is exploitable via WineBox's usage pattern.
- **Status:** New finding — 2026-07-01 review.
...
| python-multipart | >=0.0.26 | 0.0.28 | Patched for CVE-2026-40347 + CVE-2026-24486; CVE-2026-53539/53537/53538 limited impact (see INFO-26) |
...
| aiohttp | >=3.13.4 | 3.13.5 | Patched for CVE-2026-34520 + CVE-2026-34519 + CVE-2026-22815; CVE-2026-47265/34993/54274/54278 not exploitable (see INFO-19/21/33) |
...
**Codebase version:** 0.7.86 (commit 90801e4)

### Comparison with Previous Review (2026-06-30)

| Item | 2026-06-30 | 2026-07-01 | Status |
|------|-----------|-----------|--------|
...
- No application code changes since last review (HEAD at `90801e4`, previous security review commit only).
- All dependency versions unchanged.
- **INFO-26 updated:** Added two newly-disclosed python-multipart CVEs (CVE-2026-53537 Content-Disposition parameter smuggling, CVE-2026-53538 semicolon querystring separator). Both are in the same 0.0.28 version already tracked, with the same fix version (0.0.30). Neither is exploitable in WineBox due to authentication requirements, magic-byte file validation, and Pydantic input validation.
- **INFO-33 added:** Two new aiohttp CVEs (CVE-2026-54274 websocket memory bypass, CVE-2026-54278 compressed body bypass). Both affect server-side usage only; WineBox uses aiohttp as a transitive client-side dependency via the Anthropic SDK. No practical impact.
- All other 31 INFO findings carried forward unchanged.
- Full re-audit of all 12 categories confirms no regressions.

### Top 3 Priorities

1. **Bump python-multipart to >=0.0.30 (INFO-26)** — closes three CVEs (CVE-2026-53539, CVE-2026-53537, CVE-2026-53538) in a single upgrade. While none are exploitable in WineBox's current configuration, the fix is straightforward and removes all theoretical risk from parameter smuggling.
...
3. **Bump PyJWT to >=2.13.0 (INFO-18)** — closes five CVEs (CVE-2026-48526, CVE-2026-48523, CVE-2026-48525, CVE-2026-48522, CVE-2026-48524) in a single upgrade. None are exploitable today, but the upgrade is straightforward and removes all theoretical risk.
