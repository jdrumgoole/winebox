...
**Notable context:** No code changes since the last review (2026-05-26). HEAD is at commit `035c0dc` (the 2026-05-26 security review merge). A newly disclosed Starlette CVE (CVE-2026-48710, "BadHost") was evaluated against the WineBox architecture and found to have no practical impact due to the application's use of dependency-injection auth rather than path-based middleware auth.
...
**INFO-8: Starlette 0.50.0 is affected by CVE-2026-48710 (BadHost) — no practical impact on WineBox** *(New)*
...
| anthropic | >=0.96.0 | 0.96.0 | Patched for CVE-2026-34450 + CVE-2026-34452; MCP CVEs not applicable (WineBox uses API, not MCP) |
...
No dependencies are yanked, compromised, or missing critical patches. No lockfile changes since last review. No WineBox dependencies appear in 2026 supply chain compromises (litellm, PyTorch Lightning, durabletask, TanStack/Mistral AI).
...
- Only one commit since last review: `035c0dc` (merge of 2026-05-26 security review — documentation only).
...
| :yellow_circle: INFO | 8 |
| :green_circle: CLEAN | 12 categories |

**Date of review:** 2026-05-27
**Reviewer:** Automated security audit (Claude Code)
**Codebase version:** 0.7.85 (commit 035c0dc)

### Comparison with Previous Review (2026-05-26)

| Item | 2026-05-26 | 2026-05-27 | Status |
|------|-----------|-----------|--------|
...
| INFO-8 (Starlette CVE-2026-48710 BadHost) | — | New | Not exploitable in WineBox |

### Changes Since Last Review

- No code changes since last review. All dependency versions unchanged.
- Dependency CVE databases re-checked: CVE-2026-48710 (Starlette BadHost, disclosed ~2026-05-26) evaluated and found non-exploitable in WineBox's architecture (dependency-injection auth, not path-based middleware auth).
...
3. **Plan `cryptography` upgrade to 48.x (INFO-7)** — stay closer to current for defence in depth (48.0.0 ships OpenSSL 4.0.0).
