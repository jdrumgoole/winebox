...
**Notable context:** One new infrastructure commit since the last review (2026-08-06). HEAD moved from `c0f8839` to `60acc58` (version 0.7.86 unchanged). The new commit (`60acc58`) adds an `invoke prod-ssl` task for apex cert renewal and removes the dead `auto-merge-security-clean.yml` GitHub Actions workflow. No application code or dependency changes. All 56 INFO findings carried forward unchanged.
...
- One new commit since last review: `60acc58` — adds `invoke prod-ssl` task for apex cert renewal and removes the dead `auto-merge-security-clean.yml` GitHub Actions workflow. Both changes are infrastructure-only with no security impact. The `prod-ssl` task follows established patterns (`run_ssh` from `deploy.common`, `certbot certonly --standalone`, `try/finally` for nginx restart).
...
| Item | 2026-08-06 | 2026-08-12 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 0 | Maintained |
| INFO | 56 | 56 | Unchanged |

### Changes Since Last Review

- One new infrastructure commit (`60acc58`): adds `invoke prod-ssl` task for apex cert renewal, removes dead `auto-merge-security-clean.yml` workflow. No security impact — task follows established deployment patterns.
