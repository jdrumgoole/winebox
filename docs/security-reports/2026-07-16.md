...
**Notable context:** No application code changes since the last review (2026-07-15). HEAD is at `862db1d` (version 0.7.86). Dependency versions unchanged in lockfile. All 45 INFO-level findings from the previous review carried forward. Four new INFO findings added (INFO-46 through INFO-49) discovered through deeper CVE analysis of python-multipart, Pillow, aiosmtplib, and click.
...
**INFO-46: python-multipart 0.0.28 is affected by CVE-2026-53537 (Content-Disposition parameter smuggling)** :new:
...
- **Status:** New in 2026-07-16 review.

**INFO-47: Pillow 12.2.0 is affected by three additional CVEs (heap corruption, heap OOB write, EPS infinite loop) — no practical impact on WineBox** :new:
...
**INFO-48: aiosmtplib 5.1.0 is affected by CVE-2026-53533 (SMTP command injection) — limited impact on WineBox** :new:
...
**INFO-49: click 8.3.1 is affected by PYSEC-2026-2132 (command injection in `click.edit()`) — no practical impact on WineBox** :new:
...
| certifi | >=2026.4.22 | 2026.4.22 | CA bundle current; 2026.6.17 available |
...
- No code changes since last review. HEAD at `862db1d` (previous security review commit only).
...
| :yellow_circle: INFO | 49 |
| :green_circle: CLEAN | 12 categories |

**Date of review:** 2026-07-16
**Reviewer:** Automated security audit (Claude Code)
**Codebase version:** 0.7.86 (commit 862db1d)

### Comparison with Previous Review (2026-07-15)

| Item | 2026-07-15 | 2026-07-16 | Status |
|------|-----------|-----------|--------|
...
- No application code changes since last review (HEAD at `862db1d`, previous security review commit only).
- All dependency versions unchanged in lockfile.
- All 45 INFO findings carried forward unchanged.
- **4 new INFO findings** added through deeper CVE analysis:
  - **INFO-46**: CVE-2026-53537 — Content-Disposition parameter smuggling via RFC 2231/5987 decoding in python-multipart 0.0.28. This is the fourth CVE affecting the installed python-multipart version, joining INFO-26 (CVE-2026-53539), INFO-34 (CVE-2026-53540, CVE-2026-53538). All four are fixed in python-multipart 0.0.30.
  - **INFO-47**: Three additional Pillow 12.2.0 CVEs (PYSEC-2026-3453 heap corruption, PYSEC-2026-3451 heap OOB write, PYSEC-2026-3452 EPS infinite loop). None are reachable through WineBox's image pipeline. Brings the total Pillow CVE count to 7 (was 4 in INFO-38). All fixed in Pillow 12.3.0.
  - **INFO-48**: CVE-2026-53533 — SMTP command injection in aiosmtplib 5.1.0 (transitive via regstack). Limited impact — Pydantic `EmailStr` validation rejects CR/LF in addresses, and production uses SES API not SMTP. Fixed in aiosmtplib 5.1.1.
  - **INFO-49**: PYSEC-2026-2132 — Command injection in click 8.3.1's `click.edit()` function (transitive via uvicorn). No impact — WineBox never calls `click.edit()`. Fixed in click 8.3.3.
...
3. **Fix client-side escaping gaps (INFO-28, INFO-40, INFO-41)** — Upgrade the price-tracker's `escapeHtml()` to match the main app's 5-character implementation and add `escapeHtml()` to the two `app.js` locations. Low risk today, easy to fix.
