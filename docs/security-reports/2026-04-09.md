...
- `fastapi`, `pydantic`, `uvicorn`, `pymongo`, `bcrypt`, `slowapi`, `openpyxl`, `posthog` — no known CVEs in resolved versions
- No CVEs found for `fastapi` 0.115.12, `pydantic` 2.11.3, `pymongo` 4.12.1 in the last 90 days
...
- **Critical:** Both issues (#1 python-jose, #2 anthropic CVEs) remain open — no remediation since last review. `python-jose` has been flagged since 2026-04-03 (7 days without action). `anthropic` CVEs flagged since 2026-04-06 (4 days without action).
...
1. **Replace `python-jose` with `PyJWT` and bump `anthropic>=0.87.0`** — Two critical dependency issues now open for 7 and 4 days respectively. An abandoned cryptographic library and two unpatched CVEs in the Anthropic SDK. PyJWT 2.11.0 is already in the lockfile as a transitive dependency, minimizing migration effort.

2. **Implement full token invalidation on password change/reset** — Currently only the current session is revoked on password change, and no sessions are revoked on password reset. This violates the project's own CLAUDE.md security guidelines ("When changing passwords, invalidate all existing tokens for that user").

3. **Fix inconsistent `escapeHtml()` in JavaScript frontend** — Multiple XSS vectors exist in the wine detail panel, activity feed, and transaction history where user-supplied wine data is rendered without escaping.
