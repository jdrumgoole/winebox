...
**Notable context:** No application code or dependency changes since the last review (2026-07-13). HEAD is at `0afc882` (version 0.7.86). All 39 INFO-level findings from the previous review carried forward. Five new INFO findings added (INFO-40 through INFO-44) from deeper analysis in this review cycle.
...
**INFO-40: Seven list endpoints accept `skip` parameter with no upper bound** :new:

- **Files:** `winebox/routers/cellar.py:25`, `winebox/routers/met.py:17`, `winebox/routers/search.py:46`, `winebox/routers/transactions.py:32`, `winebox/routers/wines/crud.py:23`, `winebox/routers/price_tracker.py:307,380`
- **Detail:** These endpoints define `skip: int = Query(0, ge=0)` without a `le=` upper bound. While the xwines search endpoint correctly uses `ge=0, le=10000`, these seven endpoints allow arbitrarily large skip values. MongoDB must perform an index scan past skipped documents, and a very large skip (e.g., `skip=999999999`) forces unnecessary work.
- **Impact:** Very low — all queries are scoped to the authenticated user's own data, so the scan is bounded by the user's data volume. The 30-second MongoDB socket timeout provides a safety net.
- **Recommendation:** Add `le=10000` (or `le=MAX_USER_RESULTSET`) to the `skip` parameter on all seven endpoints.
- **Status:** New in 2026-07-14 review.

**INFO-41: HSTS policy inconsistency between nginx and application middleware** :new:

- **Files:** `deploy/nginx-winebox.conf:78` (`max-age=63072000`), `winebox/main.py:76-78` (`max-age=31536000; includeSubDomains; preload`)
- **Detail:** nginx sets HSTS with a 2-year max-age but without `includeSubDomains` or `preload`. The application middleware sets a 1-year max-age with `includeSubDomains; preload`. Since nginx's header is set later in the response pipeline, it takes precedence for proxied requests, meaning `includeSubDomains` and `preload` are effectively not applied.
- **Impact:** Very low — HSTS is still active with a strong max-age. The inconsistency means subdomain protection and HSTS preload are not in effect, but all WineBox subdomains already enforce HTTPS at the nginx layer.
- **Recommendation:** Standardize on a single HSTS policy. Either add `includeSubDomains; preload` to the nginx header, or remove the header from application middleware and rely solely on nginx.
- **Status:** New in 2026-07-14 review.

**INFO-42: Deprecated `datetime.utcnow()` used in test and migration files** :new:

- **Files:**
  - `tests/test_wine_ownership.py:39-40,52-53` — Uses `datetime.utcnow()` for test fixture timestamps.
  - `scripts/migrations/migrate_sqlite_to_mongo.py:264-265,375,416,440-441,465-466` — Uses `datetime.utcnow()` throughout the migration script.
- **Detail:** Both files use the deprecated timezone-naive `datetime.utcnow()` instead of the project-standard `datetime.now(timezone.utc)`. The migration script is an archived one-time tool. The test file creates timestamps for test fixtures.
- **Impact:** None — timezone-naive datetimes in these contexts do not affect production security. Violates the project's own datetime guideline.
- **Recommendation:** Update both files to use `datetime.now(timezone.utc)` for consistency.
- **Status:** New in 2026-07-14 review.

**INFO-43: Hardcoded OAT droplet IP in repair script** :new:

- **File:** `scripts/repair_oat_runner.py:53` — `root@46.101.134.8`
- **Detail:** The OAT droplet IP is hardcoded in this one-off repair script. The IP matches the documented OAT droplet IP in CLAUDE.md, so this is not environment leakage, but ideally it would be read from environment or configuration.
- **Impact:** Very low — infrastructure tooling script, not application code.
- **Recommendation:** Read the OAT IP from environment or configuration for consistency.
- **Status:** New in 2026-07-14 review.

**INFO-44: `app.js` health endpoint data inserted via `innerHTML` without escaping** :new:

- **File:** `winebox/static/js/app.js:108`
- **Detail:** The code `appInfo.innerHTML = \`${data.app_name} <span class="version">v${data.version}</span>\`` inserts values from the `/health` endpoint into the DOM via `innerHTML` without using the file's own `escapeHtml()` function. Both `data.app_name` and `data.version` are server-controlled configuration values (not user input), so this is not exploitable in practice.
- **Impact:** Very low — values come from server config, not user input. If the health endpoint were ever compromised, this would become an XSS vector, but the same compromise would provide far more direct attack paths.
- **Recommendation:** Apply `escapeHtml()` for consistency with the rest of the file's defensive coding style.
- **Status:** New in 2026-07-14 review.
...
| certifi | >=2026.4.22 | 2026.4.22 | CA bundle is current |
...
- All regex searches use `re.escape()` before compilation (verified across 18 occurrences).
...
| Item | 2026-07-13 | 2026-07-14 | Status |
|------|-----------|-----------|--------|
...
- No application code or dependency changes since last review (HEAD at `0afc882`, previous security review commit only).
- All dependency versions unchanged in lockfile.
- All 39 INFO findings carried forward unchanged.
- **5 new INFO findings** from deeper analysis in this review cycle:
  - **INFO-40:** Seven list endpoints accept `skip` parameter with no upper bound (`ge=0` only, no `le=`). All queries are user-scoped and bounded by MongoDB socket timeout. Defence-in-depth improvement.
  - **INFO-41:** HSTS policy inconsistency between nginx (`max-age=63072000`, no `includeSubDomains`) and application middleware (`max-age=31536000; includeSubDomains; preload`). nginx header takes precedence.
  - **INFO-42:** Deprecated `datetime.utcnow()` used in one test file and one archived migration script. Violates project datetime guideline but has no security impact.
  - **INFO-43:** Hardcoded OAT droplet IP in `scripts/repair_oat_runner.py`. Matches documented IP, not a leak.
  - **INFO-44:** `app.js` line 108 inserts server-controlled health endpoint data via `innerHTML` without `escapeHtml()`. Not exploitable but inconsistent with the file's defensive coding style.
...
3. **Fix escaping gaps (INFO-24, INFO-28, INFO-44)** — Upgrade admin.js and price-tracker.js `escapeHtml()` to match the main app's 5-character implementation, and add escaping to app.js line 108. Low risk today, easy to fix.
