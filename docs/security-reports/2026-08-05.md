...
**Notable context:** No application code changes since the last review (2026-08-04). HEAD is at `c9a8749` (version 0.7.86). The only change since last review is yesterday's security report being merged. Dependency versions unchanged in lockfile. Four new INFO findings (INFO-52 through INFO-55) added from deeper analysis of case/bottle event creation, nginx CSP coverage, and datetime usage in scripts. All other 51 INFO findings carried forward.
...
- Only change since last review: yesterday's security report merged (commit `c9a8749`). HEAD at `c9a8749`.
...
| :yellow_circle: INFO | 55 |
| :green_circle: CLEAN | 12 categories |

**Date of review:** 2026-08-05
**Reviewer:** Automated security audit (Claude Code)
**Codebase version:** 0.7.86 (commit c9a8749)

### Comparison with Previous Review (2026-08-04)

| Item | 2026-08-04 | 2026-08-05 | Status |
|------|-----------|-----------|--------|
| CRITICAL | 0 | 0 | Maintained |
| WARNING | 0 | 0 | Maintained |
| INFO | 51 | 55 | +4 new (INFO-52 through INFO-55) |

### Changes Since Last Review

- No application code changes since last review (HEAD at `c9a8749`, previous security review commit only).
- All dependency versions unchanged in lockfile.
- **New finding:** INFO-52 — CellarEvent creation in `cases.py` and `bottles.py` omits `owner_id` and `wine_id` fields, causing orphaned events and incomplete audit trails.
- **New finding:** INFO-53 — CSP header missing on the nginx-served `winebox.app` landing page (static content, no user data).
- **New finding:** INFO-54 — Deprecated `datetime.utcnow()` in an archived migration script and one test file.
- **New finding:** INFO-55 — `Wine.inventory.quantity` not synced by case/bottle event endpoints, causing stale stock counts.
- All other 51 INFO findings carried forward unchanged.
