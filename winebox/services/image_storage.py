...
class ImageStorageService:
    ...
    def _safe_resolve(self, filename: str) -> Path | None:
        """Resolve `filename` inside `storage_path`, refusing path traversal.

        Filenames are always UUIDs generated by `save_image`, so this is a
        defence-in-depth check: if a future caller (or DB poisoning) ever
        passes `..`/an absolute path/a symlink, the resolved path stays
        inside storage_path or this returns None.
        """
        if not filename or filename in (".", "..") or "/" in filename or "\\" in filename:
            return None
        candidate = (self.storage_path / filename).resolve()
        try:
            candidate.relative_to(self.storage_path.resolve())
        except ValueError:
            return None
        return candidate
